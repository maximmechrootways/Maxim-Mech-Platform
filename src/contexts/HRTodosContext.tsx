...
import { listHRTodos } from '@/api/hrTodos'

const STORAGE_KEY = 'maxim-hr-todos'

const EMPTY_TODOS: HRTodoItem[] = []
...
function loadTodosFromStorage(): HRTodoItem[] {
  try {
...
export function HRTodosProvider({ children }: { children: React.ReactNode }) {
  const [todos, setTodos] = useState<HRTodoItem[]>(loadTodosFromStorage)

  useEffect(() => {
    listHRTodos().then((api) => { if (api.length > 0) setTodos(api) })
  }, [])
