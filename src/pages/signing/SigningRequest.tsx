...
export function SigningRequest() {
  ...
  const requestsForLabourer = signatureRequests.filter(isCurrentUserSigner)
  const requestList = isLabourer ? requestsForLabourer : signatureRequests
  const request = id ? (isLabourer ? requestsForLabourer.find((r) => r.id === id) : signatureRequests.find((r) => r.id === id)) : null
