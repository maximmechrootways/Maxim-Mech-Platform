...
export function SubcontractorDetail() {
  ...
  const [jobsList, setJobsList] = useState<Awaited<ReturnType<typeof listJobs>>>([])
  useEffect(() => { listJobs().then(setJobsList) }, [])
  const jobAssignmentsForSub = sub ? jobAssignments.filter((a) => a.subcontractorId === sub.id) : []
  const jobs = jobAssignmentsForSub
    .map((a) => jobsList.find((j) => j.id === a.jobId))
    .filter(Boolean) as Awaited<ReturnType<typeof listJobs>>
  ...
  const [certRenewalRequested, setCertRenewalRequested] = useState(false)
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <Card padding="md">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <Button size="sm" variant="secondary" onClick={() => setCertRenewalRequested(true)}>Request cert renewal</Button>
            {certRenewalRequested && <span className="text-sm text-neutral-500 dark:text-neutral-400">(Demo: renewal request would be sent.)</span>}
          </div>
        </Card>
      </div>
