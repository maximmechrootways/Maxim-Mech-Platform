...
export function SearchResultDetail() {
  ...
  const [incident, setIncident] = useState<Awaited<ReturnType<typeof getIncident>>>(undefined)
  useEffect(() => { if (type === 'incident' && id) getIncident(id).then(setIncident) }, [type, id])
  ...
  const inc = type === 'incident' ? incident : undefined
