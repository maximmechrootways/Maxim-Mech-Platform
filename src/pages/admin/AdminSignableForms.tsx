...
export function AdminSignableForms() {
  ...
  const addForm = () => {
    ...
    if (newAssignRoles.length === 0 && newAssignUserIds.length === 0) {
      setAddFormError('Assign to at least one role or specific person.')
      return
    }
    addTemplate({
      id: `sf-${Date.now()}`,
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {showAdd && (
        <Card padding="lg">
          ...
          {addFormError && (
            <p className="mt-4 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-4 py-2 rounded-xl" role="alert">{addFormError}</p>
          )}
          <div className="mt-4 space-y-4 max-w-xl">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="ghost" onClick={() => { setShowAdd(false); setAddFormError(null) }}>Cancel</Button>
            </div>
          </div>
        </Card>
      )}
