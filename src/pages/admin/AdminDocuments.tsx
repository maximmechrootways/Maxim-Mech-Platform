...
export function AdminDocuments() {
  ...
  const addDocument = () => {
    ...
    if (newVisibility === 'restricted' && newVisibleToRoles.length === 0 && newVisibleToUserIds.length === 0) {
      setAddDocumentError('When restricted, select at least one role or person who can view.')
      return
    }
    setSaving(true)
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {showAdd && (
        <Card padding="lg">
          ...
          {addDocumentError && (
            <p className="mt-4 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-4 py-2 rounded-xl" role="alert">{addDocumentError}</p>
          )}
          <div className="mt-4 space-y-4 max-w-xl">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="ghost" onClick={() => { setShowAdd(false); setAddDocumentError(null) }}>Cancel</Button>
            </div>
          </div>
        </Card>
      )}
