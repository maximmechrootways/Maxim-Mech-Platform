...
export function FormFromPdfEditor() {
  ...
  const save = () => {
    ...
    if (!hasSignature) {
      setSaveError('Add at least one Signature field so the submitter can sign by text.')
      return
    }
    if (assignedToRoles.length === 0 && assignedToUserIds.length === 0) {
      setSaveError('Assign to at least one role or specific person.')
      return
    }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {saveError && (
        <p className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-4 py-2 rounded-xl" role="alert">
          {saveError}
        </p>
      )}
