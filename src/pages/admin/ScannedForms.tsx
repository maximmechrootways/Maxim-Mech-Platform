...
export function ScannedForms() {
  ...
  const { pdfs, addPdf: addPdfToContext } = useScannedPdfs()
  ...
  const formsFromPdf = (pdfId: string) =>
    templates.filter((t) => t.sourcePdfId === pdfId)

  const addPdf = () => {
    const name = `Scanned form ${pdfs.length + 1}.pdf`
    addPdfToContext({ id: `pdf-${Date.now()}`, name, uploadedAt: new Date().toISOString().slice(0, 19) + 'Z', uploadedBy: 'User' })
  }
