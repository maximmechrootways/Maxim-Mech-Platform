...
import { listInspectionSchedules, listInspectionResults } from '@/api/inspections'

export function ScheduledInspections() {
  const [schedules, setSchedules] = useState<Awaited<ReturnType<typeof listInspectionSchedules>>>([])
  const [results, setResults] = useState<Awaited<ReturnType<typeof listInspectionResults>>>([])
  useEffect(() => { listInspectionSchedules().then(setSchedules) }, [])
  useEffect(() => { listInspectionResults().then(setResults) }, [])
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Card padding="lg">
        ...
        {schedules.length === 0 ? (
          <EmptyState title="No scheduled inspections" description="Add inspection schedules in Admin." />
        ) : (
