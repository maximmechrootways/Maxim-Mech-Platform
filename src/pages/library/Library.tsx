...
import { listSignatureRequests } from '@/api/signatureRequests'
import { listFormTemplates } from '@/api/formTemplates'
...
export function Library() {
  ...
  useEffect(() => { listSignatureRequests().then(setSignatureRequests) }, [])
  useEffect(() => { listFormTemplates().then(setFormTemplates) }, [])
  ...
  const signingRequests = useMemo(() => {
    const isSigner = (r: SignatureRequest) =>
      r.requiredSigners.some((s) => s.userId === user?.id || s.name === user?.name)
    return isLabourer ? signatureRequests.filter(isSigner) : signatureRequests
  }, [signatureRequests, user, isLabourer])
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {view === 'templates' && (
        <>
          ...
          {/* Report forms from API (incident, near-miss, hazard) */}
          <div className="mb-6">
            <h3 className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-3">Report forms</h3>
            <ul className="space-y-3">
              {formTemplates
                .filter((t) => t.category === 'incident' || t.category === 'near_miss' || t.category === 'hazard')
                .map((t) => (
                  <li key={t.id}>
                    <Card hover padding="md" className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                      ...
                      <Link to={`/forms/new/${t.id}`}>
                        <Button size="sm">{t.category === 'incident' ? 'Report incident' : t.category === 'near_miss' ? 'Report near-miss' : 'Report hazard'}</Button>
                      </Link>
                    </Card>
                  </li>
                ))}
            </ul>
            {formTemplates.filter((t) => ['incident', 'near_miss', 'hazard'].includes(t.category ?? '')).length === 0 && (
              <p className="text-sm text-neutral-500 dark:text-neutral-400">No report forms available. Configure form templates in Admin.</p>
            )}
          </div>
