...
import { getDailyForm } from '@/api/dailyForms'
import type { AppUserSummary } from '@/api/users'

/** Labourers assigned to jobs where the given user is a supervisor */
function labourersForSupervisor(
  supervisorId: string,
  jobs: Awaited<ReturnType<typeof listJobs>>,
  assignments: Awaited<ReturnType<typeof listJobAssignments>>,
  appUsers: AppUserSummary[]
) {
  const jobIds = jobs.filter((j) => (j.assignedSupervisorIds ?? []).includes(supervisorId)).map((j) => j.id)
  const labourerIds = new Set(assignments.filter((a) => jobIds.includes(a.jobId)).map((a) => a.userId))
  return appUsers.filter((u) => u.role === 'labourer' && labourerIds.has(u.id))
}
...
export function FillSignableForm() {
  ...
  const [dailyForm, setDailyForm] = useState<Awaited<ReturnType<typeof getDailyForm>>>(undefined)
  ...
  useEffect(() => { if (dailyFormId) getDailyForm(dailyFormId).then(setDailyForm) }, [dailyFormId])
  ...
  const myLabourers = useMemo(() => (user?.id ? labourersForSupervisor(user.id, jobs, assignments, appUsers) : []), [user?.id, jobs, assignments, appUsers])
  ...
  const submit = () => {
    ...
    if (missing.length > 0) {
      setSubmitError(`Please fill: ${missing.map((f) => f.label).join(', ')}`)
      return
    }
    if (sendToLabourers && selectedLabourerIds.length === 0) {
      setSubmitError('Select at least one labourer to send the form to for signing.')
      return
    }
    setLoading(true)
    captureGeo()
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
      ...
      {submitError && (
        <p className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-4 py-2 rounded-xl" role="alert">{submitError}</p>
      )}
