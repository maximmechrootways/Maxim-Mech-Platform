...
import type { FormSubmissionStatus, FormTemplate } from '@/types'
import { UNKNOWN_TEMPLATE } from '@/data/defaults'
import { getFormTemplatesRecord } from '@/api/formTemplates'
...
export function FormReview() {
  ...
  const [templatesRecord, setTemplatesRecord] = useState<Record<string, FormTemplate>>({})
  const [appUsers, setAppUsers] = useState<AppUserSummary[]>([])
  useEffect(() => { getFormTemplatesRecord().then(setTemplatesRecord) }, [])
  useEffect(() => { listUsers().then(setAppUsers) }, [])
  const template: FormTemplate = submission ? (templatesRecord[submission.templateId] ?? UNKNOWN_TEMPLATE) : UNKNOWN_TEMPLATE
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-fade-in">
        ...
        <Card key={section.id} padding="lg">
          ...
          <div className="space-y-3 text-sm">
            {section.fields.map((f) => {
              const raw = submission?.fieldValues?.[f.id]
      ...
      {/* Hazard form: supervisor signatures (multiple name/date) */}
      {submission?.templateId === 't5' && submission?.fieldValues && (() => {
        const sigs: { name: string; date: string }[] = []
        let i = 0
        while (true) {
          const name = submission.fieldValues[`hz_sig_${i}_name`]
          const date = submission.fieldValues[`hz_sig_${i}_date`]
          if (name == null && date == null) break
          sigs.push({
            name: typeof name === 'string' ? name : '',
            date: typeof date === 'string' ? date : (date != null ? String(date) : ''),
          })
          i++
        }
        if (sigs.length === 0) return null
        return (
          <Card key="hz-sigs" padding="lg">
            <h2 className="font-display font-semibold text-lg text-neutral-900 dark:text-white mb-4">Supervisor signatures</h2>
            <ul className="space-y-2 text-sm">
              {sigs.map((sig, idx) => (
                <li key={idx} className="flex flex-wrap gap-x-4 gap-y-0.5 text-neutral-700 dark:text-neutral-300">
                  <span className="font-medium">{sig.name || '—'}</span>
                  <span className="text-neutral-500 dark:text-neutral-400">{sig.date ? new Date(sig.date).toLocaleDateString() : ''}</span>
                </li>
              ))}
            </ul>
          </Card>
