...
export function DashboardLabourer() {
  const { user } = useUser()
  const [signatureRequests, setSignatureRequests] = useState<Awaited<ReturnType<typeof listSignatureRequests>>>([])
  ...
  const needSignature = signatureRequests.filter((sr) => sr.requiredSigners.some((s) => s.status === 'pending'))
  const supervisorName = 'Your supervisor'
  const myAssignmentIds = assignments.filter((a) => a.userId === user?.id).map((a) => a.jobId)
  const assignedJobs = jobs.filter((j) => myAssignmentIds.includes(j.id)).map((j) => ({ id: j.id, title: j.title, site: j.siteName ?? '', due: '' }))
