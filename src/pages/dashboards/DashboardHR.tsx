...
import { listDailyFormsToComplete } from '@/api/dailyForms'
import { listHazardReports } from '@/api/hazards'
import { listComplianceCalendar } from '@/api/complianceCalendar'
import type { Job, JobCheckIn } from '@/types'
...
export function DashboardHR() {
  const [scheduleReportSent, setScheduleReportSent] = useState(false)
  const [jobs, setJobs] = useState<Job[]>([])
  const [checkIns, setCheckIns] = useState<JobCheckIn[]>([])
  const [dailyForms, setDailyForms] = useState<Awaited<ReturnType<typeof listDailyFormsToComplete>>>([])
  const [hazardReports, setHazardReports] = useState<Awaited<ReturnType<typeof listHazardReports>>>([])
  const [complianceCalendar, setComplianceCalendar] = useState<Awaited<ReturnType<typeof listComplianceCalendar>>>([])
  const [injuriesByMonth, setInjuriesByMonth] = useState<{ month: string; reported: number; closed: number }[]>([])
  const [formsCompletion, setFormsCompletion] = useState<{ week: string; completed: number; due: number }[]>([])
  useEffect(() => { listJobs().then(setJobs) }, [])
  useEffect(() => { listJobCheckIns().then(setCheckIns) }, [])
  useEffect(() => { listDailyFormsToComplete().then(setDailyForms) }, [])
  useEffect(() => { listHazardReports().then(setHazardReports) }, [])
  useEffect(() => { listComplianceCalendar().then(setComplianceCalendar) }, [])
  ...
  const complianceDue = complianceCalendar.filter((e) => e.dueDate >= now.toISOString().slice(0, 10)).slice(0, 5).length
  const { reports: injuryReports } = useInjuryReports()
  const openInjuries = injuryReports.filter((r) => r.status !== 'closed')
  const activeJobs = jobs.filter((j) => j.status === 'active').length
  const checkedInToday = checkIns.filter((c) => c.date === TODAY && c.checkedInAt).length
  const totalAssignedToday = checkIns.filter((c) => c.date === TODAY).length
  const formsPending = dailyForms.filter((f) => f.status !== 'signed').length
  const customForms = templates.filter((f) => f.active)
  const openHazards = hazardReports.filter((h) => h.status !== 'closed').length
  ...
  const formCompletionRate = formsCompletion.length ? Math.round((formsCompletion.reduce((s, w) => s + w.completed, 0) / Math.max(1, formsCompletion.reduce((s, w) => s + w.due, 0))) * 100) : 0
    ...
    <div className="animate-fade-in">
      ...
      <header className="mb-8">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          <button type="button" className="text-sm text-neutral-500 dark:text-neutral-400 hover:underline" onClick={() => setScheduleReportSent(true)}>Schedule weekly report</button>
          {scheduleReportSent && <span className="text-sm text-neutral-500 dark:text-neutral-400 ml-2">(Demo: would email weekly.)</span>}
        ...
        <Card padding="md">
          ...
          <div className="mt-4 h-[240px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={injuriesByMonth} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
      ...
      <section>
        <Card padding="md">
          ...
          <div className="mt-3 h-[160px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={formsCompletion} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" className="stroke-neutral-200 dark:stroke-neutral-600" vertical={false} />
