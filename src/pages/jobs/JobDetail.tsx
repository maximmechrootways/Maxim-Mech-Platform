...
import { listJobs, getJob, listJobAssignments, listJobCheckIns, getEmergencySiteInfo } from '@/api/jobs'
...
export function JobDetail() {
  const { id } = useParams()
  const { user } = useUser()
  const [job, setJob] = useState<Awaited<ReturnType<typeof getJob>>>(undefined)
  const [assignments, setAssignments] = useState<Awaited<ReturnType<typeof listJobAssignments>>>([])
  const [checkIns, setCheckIns] = useState<Awaited<ReturnType<typeof listJobCheckIns>>>([])
  const [appUsers, setAppUsers] = useState<Awaited<ReturnType<typeof listUsers>>>([])
  const [emergencyInfo, setEmergencyInfo] = useState<Awaited<ReturnType<typeof getEmergencySiteInfo>>>(undefined)
  useEffect(() => { if (id) getJob(id).then(setJob) }, [id])
  useEffect(() => { listJobAssignments(id).then(setAssignments) }, [id])
  useEffect(() => { listJobCheckIns(id).then(setCheckIns) }, [id])
  useEffect(() => { listUsers().then(setAppUsers) }, [])
  useEffect(() => { if (id) getEmergencySiteInfo(id).then(setEmergencyInfo) }, [id])
  const assignableLabourers = appUsers.filter((u) => u.role === 'labourer')
  ...
  // Only show labourers who are not on leave and not terminated
      ...
      <Card padding="lg">
        ...
        <ul className="mt-4 space-y-2">
          {jobLabourers.length === 0 ? (
            ...
            jobLabourers.map((a) => {
              const labourer = appUsers.find((l) => l.id === a.userId)
        ...
        {canAssignLabourers && (
          <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-600">
            ...
            <div className="flex flex-wrap gap-2">
              {assignableLabourers.filter((l) => !labourerIds.includes(l.id)).map((l) => (
                <Button key={l.id} size="sm" variant="outline" onClick={() => assignLabourer(l.id)}>
                  + {l.name}
                </Button>
              ))}
              {assignableLabourers.filter((l) => !labourerIds.includes(l.id)).length === 0 && (
                <span className="text-sm text-neutral-500">All assignable labourers assigned (people on leave are not shown).</span>
              )}
            </div>
          </div>
        )}
