...
import type { Job, JobAssignment, JobTemplate } from '@/types'
import { listJobs, listJobAssignments, listJobTemplates } from '@/api/jobs'
...
export function JobManagement() {
  ...
  useEffect(() => { listJobAssignments().then(setJobAssignments) }, [])
  useEffect(() => { listJobTemplates().then(setJobTemplates) }, [])
  useEffect(() => { listUsers().then((u) => setSupervisors(u.filter((x) => x.role === 'supervisor'))) }, [])

  const isOwnerOrHr = user?.role === 'owner' || user?.role === 'hr'

  const getAssignmentCount = (jobId: string) =>
    jobAssignments.filter((a) => a.jobId === jobId).length
