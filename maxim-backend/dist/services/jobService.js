...
exports.getLabourerIdsSupervisedBy = getLabourerIdsSupervisedBy;
...
exports.addLabourer = addLabourer;
exports.removeLabourer = removeLabourer;
exports.addSubcontractor = addSubcontractor;
exports.removeSubcontractor = removeSubcontractor;
exports.checkIn = checkIn;
exports.resetCheckIn = resetCheckIn;
...
async function listJobs(userId, userRole, query) {
    const where = {};
    if (query.status)
...
async function getLabourerIdsSupervisedBy(supervisorUserId) {
    const assignments = await prisma_1.prisma.jobAssignment.findMany({
        where: {
            OR: [
    ...
    const siteAssignments = await prisma_1.prisma.siteAssignment.findMany({
...
async function getMyTeamMembers(userId, userRole) {
...
async function createJob(userId, userRole, data) {
    if (!isOwnerOrHr(userRole))
        throw { status: 403, message: 'Only Owner or HR can create jobs' };
    const site = await prisma_1.prisma.site.findUnique({ where: { id: data.siteId } });
    if (!site)
...
async function getJobById(id, userId, userRole) {
...
async function updateJob(id, userId, userRole, data) {
    if (!isOwnerOrHr(userRole))
        throw { status: 403, message: 'Only Owner or HR can update jobs' };
    ...
    if (data.gate !== undefined)
        updateData.gate = data.gate.trim() || null;
...
async function deleteJob(id, userId, userRole) {
    if (!isOwnerOrHr(userRole))
        throw { status: 403, message: 'Only Owner or HR can delete jobs' };
    const job = await prisma_1.prisma.job.findUnique({ where: { id } });
    if (!job)
        throw { status: 404, message: 'Job not found' };
...
async function addSupervisor(jobId, userId, userRole, body) {
    if (!isOwnerOrHr(userRole))
        throw { status: 403, message: 'Only Owner or HR can assign supervisors' };
    const job = await prisma_1.prisma.job.findUnique({ where: { id: jobId }, select: { id: true, siteId: true } });
    if (!job)
        throw { status: 404, message: 'Job not found' };
    ...
    await prisma_1.prisma.siteSupervisor.upsert({
...
async function removeSupervisor(jobId, targetUserId, userId, userRole) {
    if (!isOwnerOrHr(userRole))
        throw { status: 403, message: 'Only Owner or HR can remove supervisors' };
    const job = await prisma_1.prisma.job.findUnique({ where: { id: jobId }, select: { siteId: true } });
    if (!job)
        throw { status: 404, message: 'Job not found' };
    await prisma_1.prisma.jobSupervisor.deleteMany({ where: { jobId, userId: targetUserId } });
    await prisma_1.prisma.siteSupervisor.deleteMany({ where: { siteId: job.siteId, userId: targetUserId } });
    return { message: 'Supervisor removed' };
}
async function addLabourer(jobId, userId, userRole, body) {
    const job = await prisma_1.prisma.job.findUnique({ where: { id: jobId }, include: { supervisors: true } });
    if (!job)
        throw { status: 404, message: 'Job not found' };
    const isSupervisorOnJob = job.supervisors.some((s) => s.userId === userId);
    if (!isOwnerOrHr(userRole) && !isSupervisorOnJob)
        throw { status: 403, message: 'Only Owner, HR, or assigned Supervisor can assign labourers' };
    await prisma_1.prisma.jobAssignment.upsert({
        where: { jobId_userId: { jobId, userId: body.userId } },
...
async function removeLabourer(jobId, targetUserId, userId, userRole) {
    const job = await prisma_1.prisma.job.findUnique({ where: { id: jobId }, include: { supervisors: true } });
    if (!job)
        throw { status: 404, message: 'Job not found' };
    const isSupervisorOnJob = job.supervisors.some((s) => s.userId === userId);
    if (!isOwnerOrHr(userRole) && !isSupervisorOnJob)
        throw { status: 403, message: 'Forbidden' };
    await prisma_1.prisma.jobAssignment.deleteMany({ where: { jobId, userId: targetUserId } });
    return { message: 'Labourer removed' };
}
async function addSubcontractor(jobId, userId, userRole, body) {
    if (!isOwnerOrHr(userRole))
        throw { status: 403, message: 'Only Owner or HR can assign subcontractors' };
    const job = await prisma_1.prisma.job.findUnique({ where: { id: jobId } });
    if (!job)
        throw { status: 404, message: 'Job not found' };
    const sub = await prisma_1.prisma.subcontractor.findUnique({ where: { id: body.subcontractorId } });
    if (!sub)
        throw { status: 404, message: 'Subcontractor not found' };
    await prisma_1.prisma.subcontractorJobAssignment.upsert({
...
async function removeSubcontractor(jobId, subcontractorId, userId, userRole) {
    if (!isOwnerOrHr(userRole))
        throw { status: 403, message: 'Only Owner or HR can remove subcontractors' };
    await prisma_1.prisma.subcontractorJobAssignment.deleteMany({ where: { jobId, subcontractorId } });
    return { message: 'Subcontractor removed' };
}
async function checkIn(jobId, userId, userRole, body) {
    const job = await prisma_1.prisma.job.findUnique({ where: { id: jobId }, include: { supervisors: true, labourers: true } });
    if (!job)
        throw { status: 404, message: 'Job not found' };
    const isSupervisorOnJob = job.supervisors.some((s) => s.userId === userId);
    const isLabourerOnJob = job.labourers.some((l) => l.userId === body.targetUserId);
    if (!isLabourerOnJob)
        throw { status: 400, message: 'User is not assigned to this job' };
    if (!isOwnerOrHr(userRole) && !isSupervisorOnJob)
        throw { status: 403, message: 'Only Supervisor or HR can record check-in' };
    const date = body.date || new Date().toISOString().slice(0, 10);
    let record = await prisma_1.prisma.jobCheckIn.findUnique({
        where: { jobId_userId_date: { jobId, userId: body.targetUserId, date } },
    });
    if (!record) {
        record = await prisma_1.prisma.jobCheckIn.create({
    ...
    if (!record.checkedInAt) {
        await prisma_1.prisma.jobCheckIn.update({
            where: { id: record.id },
...
async function resetCheckIn(jobId, userId, userRole, body) {
    const job = await prisma_1.prisma.job.findUnique({ where: { id: jobId }, include: { supervisors: true } });
    if (!job)
        throw { status: 404, message: 'Job not found' };
    const isSupervisorOnJob = job.supervisors.some((s) => s.userId === userId);
    if (!isOwnerOrHr(userRole) && !isSupervisorOnJob)
        throw { status: 403, message: 'Only Supervisor or HR can reset check-in' };
    const date = body.date || new Date().toISOString().slice(0, 10);
    await prisma_1.prisma.jobCheckIn.deleteMany({ where: { jobId, userId: body.targetUserId, date } });
