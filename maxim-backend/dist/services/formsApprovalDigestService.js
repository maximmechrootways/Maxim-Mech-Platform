...
exports.buildDigestEmail = buildDigestEmail;
exports.collectPreviousDayPendingItems = collectPreviousDayPendingItems;
exports.startFormsApprovalDigestWorker = startFormsApprovalDigestWorker;
exports.enqueueTestFormsApprovalDigestForUser = enqueueTestFormsApprovalDigestForUser;
...
function getNowPartsInTimezone(now, timeZone) {
...
async function collectPreviousDayPendingItems(now) {
    ...
    const [formRows, pdfRows, signableRows] = await Promise.all([
        ...
        prisma_1.prisma.pdfSubmission.findMany({
            where: {
        ...
        prisma_1.prisma.signableFormSubmission.findMany({
            where: {
    ...
    for (const row of formRows) {
        if (!row.submittedAt)
            continue;
        if (toDateKeyInTimezone(row.submittedAt, DIGEST_TIMEZONE) !== yesterdayKey)
            continue;
    ...
    for (const row of pdfRows) {
        ...
        if (toDateKeyInTimezone(effectiveTime, DIGEST_TIMEZONE) !== yesterdayKey)
            continue;
        const submitter = row.submittedBy
            ? `${row.submittedBy.firstName || ''} ${row.submittedBy.lastName || ''}`.trim() || row.submittedBy.email
            : 'Unknown';
    ...
    for (const row of signableRows) {
        if (!row.submittedToHrAt)
            continue;
        if (toDateKeyInTimezone(row.submittedToHrAt, DIGEST_TIMEZONE) !== yesterdayKey)
            continue;
...
async function sendWeekdayMorningDigest(now) {
    ...
    for (const recipient of recipients) {
        if (!recipient.email)
            continue;
        const prefs = (0, uiPreferencesService_1.normalizeUiPreferences)(recipient.uiPreferences).notificationPreferences;
        if (!prefs.digest_hr_owner_8am)
            continue;
...
function getTodayDateKeyInTimezone(now) {
...
function startFormsApprovalDigestWorker() {
    if (intervalRef)
        return;
    tick().catch((error) => {
 ...
 */
async function enqueueTestFormsApprovalDigestForUser(userId) {
    ...
    if (!user.email?.trim())
        throw { status: 400, message: 'User has no email on file' };
