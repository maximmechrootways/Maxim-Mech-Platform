...
exports.checkAndNotifyNicheBakeryQuarterlyForms = checkAndNotifyNicheBakeryQuarterlyForms;
exports.startNicheBakeryReminderWorker = startNicheBakeryReminderWorker;
...
async function checkAndNotifyNicheBakeryQuarterlyForms(now = new Date()) {
    if (!isQuarterlyTriggerDate(now))
        return;
    const quarterKey = getQuarterKey(now);
    const monthLabel = getMonthLabel(now.getUTCMonth());
    const nicheJobs = await prisma_1.prisma.job.findMany({
        where: {
            status: 'active',
            OR: [
    ...
    for (const job of nicheJobs) {
        ...
        for (const supervisor of supervisors) {
            ...
            const body = `Please complete all Niche Bakery quarterly forms for ${job.title} (${siteName}). ` +
...
function startNicheBakeryReminderWorker() {
    if (intervalRef)
        return;
