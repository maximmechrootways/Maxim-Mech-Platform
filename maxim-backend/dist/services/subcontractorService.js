...
exports.listSubcontractors = listSubcontractors;
exports.listAllSubcontractorCertifications = listAllSubcontractorCertifications;
...
exports.upsertSubcontractorHSManualPdf = upsertSubcontractorHSManualPdf;
exports.upsertSubcontractorWsibPdf = upsertSubcontractorWsibPdf;
exports.upsertSubcontractorHrSafetyPdf = upsertSubcontractorHrSafetyPdf;
exports.upsertSubcontractorForm1000Pdf = upsertSubcontractorForm1000Pdf;
exports.addSubcontractorInsurance = addSubcontractorInsurance;
exports.deleteSubcontractorInsurance = deleteSubcontractorInsurance;
exports.deleteSubcontractor = deleteSubcontractor;
exports.addSubcontractorCertification = addSubcontractorCertification;
exports.updateSubcontractorCertification = updateSubcontractorCertification;
exports.removeSubcontractorCertification = removeSubcontractorCertification;
exports.addSubcontractorContract = addSubcontractorContract;
exports.removeSubcontractorContract = removeSubcontractorContract;
exports.listSubcontractorPersonnel = listSubcontractorPersonnel;
exports.addSubcontractorPersonnel = addSubcontractorPersonnel;
exports.updateSubcontractorPersonnel = updateSubcontractorPersonnel;
exports.removeSubcontractorPersonnel = removeSubcontractorPersonnel;
exports.addPersonnelJobAssignment = addPersonnelJobAssignment;
exports.updatePersonnelJobAssignment = updatePersonnelJobAssignment;
exports.removePersonnelJobAssignment = removePersonnelJobAssignment;
exports.addPersonnelCertification = addPersonnelCertification;
exports.updatePersonnelCertification = updatePersonnelCertification;
exports.removePersonnelCertification = removePersonnelCertification;
exports.addPersonnelDocument = addPersonnelDocument;
exports.removePersonnelDocument = removePersonnelDocument;
exports.checkInSubcontractorPersonnel = checkInSubcontractorPersonnel;
exports.checkOutSubcontractorPersonnel = checkOutSubcontractorPersonnel;
exports.listSubcontractorPersonnelCheckIns = listSubcontractorPersonnelCheckIns;
...
function isWsibReportOptionalFromNotes(notes) {
    return String(notes ?? '').includes(WSIB_OPTIONAL_MARKER);
}
function isWsibClearanceOptionalFromNotes(notes) {
    return String(notes ?? '').includes(WSIB_CLEARANCE_OPTIONAL_MARKER);
}
function isForm1000OptionalFromNotes(notes) {
    return String(notes ?? '').includes(FORM1000_OPTIONAL_MARKER);
}
function stripOptionalNoteMarkers(notes) {
    return String(notes ?? '')
...
function buildNotesWithOptionalMarkers(userNotes, reportOptional, clearanceOptional, form1000Optional) {
    const base = userNotes;
    const parts = [base, reportOptional && WSIB_OPTIONAL_MARKER, clearanceOptional && WSIB_CLEARANCE_OPTIONAL_MARKER, form1000Optional && FORM1000_OPTIONAL_MARKER].filter(Boolean);
...
function calculateCompliance(subcontractor, certifications, contracts, insurances) {
    const missing = [];
    const expiringSoon = [];
    ...
    insurances.forEach(ins => {
        if (ins.type === 'WSIB' && wsibClearanceOptional)
            return;
        if (ins.expiresAt) {
            const today = new Date().toISOString().slice(0, 10);
            const in30 = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
            if (ins.expiresAt < today) {
                if (!missing.includes(`${ins.type} Insurance (Expired)`))
                    missing.push(`${ins.type} Insurance (Expired)`);
            }
            else if (ins.expiresAt <= in30) {
                if (!expiringSoon.includes(`${ins.type} Insurance`))
                    expiringSoon.push(`${ins.type} Insurance`);
            }
        }
    });
...
async function getSubcontractorMapWithCompliance(id) {
    const s = await prisma_1.prisma.subcontractor.findUnique({
        where: { id },
        include: {
...
async function listSubcontractors(userId, userRole) {
    if (userRole === 'owner' || userRole === 'hr') {
        const list = await prisma_1.prisma.subcontractor.findMany({
    ...
    if (userRole === 'supervisor') {
        ...
        const assignments = await prisma_1.prisma.subcontractorJobAssignment.findMany({
        ...
        const listWithCompliance = await prisma_1.prisma.subcontractor.findMany({
        ...
        const subMap = new Map(listWithCompliance.map(s => [s.id, s]));
        return list.map((s) => {
            const fullS = subMap.get(s.id);
            const allCerts = [
                ...(fullS.certifications || []),
                ...(fullS.personnel?.flatMap((p) => p.certifications) || [])
            ];
...
async function listAllSubcontractorCertifications(userId, userRole) {
    ...
    const companyCertifications = await prisma_1.prisma.subcontractorCertification.findMany({
        where: subFilter
    });
    ...
    if (userRole === 'supervisor') {
        personnelSubFilter = {
            personnel: {
                subcontractorId: { in: subFilter.subcontractorId.in }
            }
        };
    }
    const personnelCertifications = await prisma_1.prisma.subcontractorPersonnelCertification.findMany({
        where: {
            ...personnelSubFilter,
            personnel: {
                ...personnelSubFilter.personnel,
...
async function getSubcontractorById(id, userId, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr' && userRole !== 'supervisor') {
        throw { status: 403, message: 'Forbidden' };
    }
    const sub = await prisma_1.prisma.subcontractor.findUnique({
        where: { id },
        include: {
            contracts: true,
            certifications: true,
    ...
    if (userRole === 'supervisor') {
        ...
        const isManager = sub.jobAssignments.some(a => a.job.supervisors && a.job.supervisors.some(s => s.userId === userId));
        if (!isManager) {
            throw { status: 403, message: 'Forbidden' };
        }
    }
    const jobs = sub.jobAssignments.map((a) => ({
        ...
        siteId: a.job?.site?.name || a.job?.siteId,
        assignedAt: a.assignedAt?.toISOString?.() ?? undefined,
    }));
    const injuries = await prisma_1.prisma.injuryReport.findMany({
    ...
    return {
        ...
        insurances: sub.insurances?.map(mapInsurance) ?? [],
        jobAssignments: jobs,
...
async function createSubcontractor(userId, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can create subcontractors' };
    const sub = await prisma_1.prisma.subcontractor.create({
        data: {
...
async function updateSubcontractor(id, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can update subcontractors' };
    const existing = await prisma_1.prisma.subcontractor.findUnique({
        where: { id },
        select: { notes: true },
    });
    if (!existing)
        throw { status: 404, message: 'Subcontractor not found' };
    const targetReportOptional = data.wsibInjuryReportOptional !== undefined
        ? data.wsibInjuryReportOptional
        : isWsibReportOptionalFromNotes(existing.notes);
    const targetClearanceOptional = data.wsibClearanceOptional !== undefined
        ? data.wsibClearanceOptional
        : isWsibClearanceOptionalFromNotes(existing.notes);
    const targetForm1000Optional = data.form1000Optional !== undefined
        ? data.form1000Optional
        : isForm1000OptionalFromNotes(existing.notes);
    const userNotes = stripOptionalNoteMarkers(data.notes !== undefined ? data.notes : existing.notes);
    const resolvedNotes = buildNotesWithOptionalMarkers(userNotes, targetReportOptional, targetClearanceOptional, targetForm1000Optional);
    ...
    await prisma_1.prisma.subcontractor.update({
        where: { id },
        data: {
...
async function upsertSubcontractorHSManualPdf(subcontractorId, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can manage H&S Manual PDFs' };
    const existing = await prisma_1.prisma.subcontractor.findUnique({ where: { id: subcontractorId } });
    if (!existing)
        throw { status: 404, message: 'Subcontractor not found' };
    const oldFilePath = existing.hsPdfFilePath ?? undefined;
    const updated = await prisma_1.prisma.subcontractor.update({
...
async function upsertSubcontractorWsibPdf(subcontractorId, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can manage WSIB documents' };
    const existing = await prisma_1.prisma.subcontractor.findUnique({ where: { id: subcontractorId } });
    if (!existing)
        throw { status: 404, message: 'Subcontractor not found' };
...
async function upsertSubcontractorHrSafetyPdf(subcontractorId, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can manage HR Safety documents' };
    const existing = await prisma_1.prisma.subcontractor.findUnique({ where: { id: subcontractorId } });
    if (!existing)
        throw { status: 404, message: 'Subcontractor not found' };
...
async function upsertSubcontractorForm1000Pdf(subcontractorId, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can manage FORM 1000 documents' };
    const existing = await prisma_1.prisma.subcontractor.findUnique({ where: { id: subcontractorId } });
    if (!existing)
        throw { status: 404, message: 'Subcontractor not found' };
...
async function addSubcontractorInsurance(subcontractorId, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can manage insurances' };
    const sub = await prisma_1.prisma.subcontractor.findUnique({ where: { id: subcontractorId } });
    if (!sub)
        throw { status: 404, message: 'Subcontractor not found' };
    ...
    if (data.expiresAt) {
        const today = new Date().toISOString().slice(0, 10);
        if (data.expiresAt < today)
            status = 'expired';
    }
    const insurance = await prisma_1.prisma.subcontractorInsurance.create({
        data: {
            subcontractorId,
            type: data.type.trim(),
...
async function deleteSubcontractorInsurance(insuranceId, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can manage insurances' };
    const existing = await prisma_1.prisma.subcontractorInsurance.findUnique({ where: { id: insuranceId } });
    if (!existing)
        throw { status: 404, message: 'Insurance not found' };
    await prisma_1.prisma.subcontractorInsurance.delete({ where: { id: insuranceId } });
    return existing; // return so router can unlink file if needed
}
async function deleteSubcontractor(id, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can delete subcontractors' };
    await prisma_1.prisma.subcontractor.delete({ where: { id } }).catch(() => {
        throw { status: 404, message: 'Subcontractor not found' };
    });
    return { message: 'Deleted' };
}
async function addSubcontractorCertification(subcontractorId, userRole, data, uploaderId, uploaderName) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can manage subcontractor certifications' };
    const sub = await prisma_1.prisma.subcontractor.findUnique({ where: { id: subcontractorId } });
    if (!sub)
        throw { status: 404, message: 'Subcontractor not found' };
    const status = certStatus(data.expiresAt);
    const cert = await prisma_1.prisma.subcontractorCertification.create({
        data: {
            subcontractorId,
            name: data.name.trim(),
...
async function updateSubcontractorCertification(id, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can update subcontractor certifications' };
    const existing = await prisma_1.prisma.subcontractorCertification.findUnique({ where: { id } });
    if (!existing)
        throw { status: 404, message: 'Certification not found' };
    const expiresAt = data.expiresAt ?? existing.expiresAt;
    const status = certStatus(expiresAt);
    const cert = await prisma_1.prisma.subcontractorCertification.update({
        where: { id },
        data: {
    ...
    try {
        ...
        if (data.issuedAt !== undefined)
            updateData.issueDate = data.issuedAt.trim() || null;
        if (data.expiresAt !== undefined)
            updateData.expirationDate = data.expiresAt.trim();
        if (Object.keys(updateData).length > 0) {
            if (existing.filePath) {
                await prisma_1.prisma.certificate.updateMany({
...
async function removeSubcontractorCertification(id, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can remove subcontractor certifications' };
    const existing = await prisma_1.prisma.subcontractorCertification.findUnique({ where: { id } });
    if (!existing)
        throw { status: 404, message: 'Certification not found' };
    await prisma_1.prisma.subcontractorCertification.delete({ where: { id } });
...
async function addSubcontractorContract(subcontractorId, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can manage contracts' };
    ...
    const personnelId = data.personnelId && data.personnelId.trim() !== '' && data.personnelId !== 'null' && data.personnelId !== 'undefined'
        ...
        : null;
    const contract = await prisma_1.prisma.subcontractorContract.create({
        data: {
            subcontractorId,
            personnelId,
...
async function removeSubcontractorContract(contractId, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can manage contracts' };
    const contract = await prisma_1.prisma.subcontractorContract.findUnique({ where: { id: contractId } });
    if (!contract)
        throw { status: 404, message: 'Contract not found' };
    await prisma_1.prisma.subcontractorContract.delete({ where: { id: contractId } });
    return contract; // Returning contract to let router know if physical file needs to be unlinked
}
// ================= Personnel =================
async function listSubcontractorPersonnel(subcontractorId, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr' && userRole !== 'supervisor')
        throw { status: 403, message: 'Forbidden' };
    const personnel = await prisma_1.prisma.subcontractorPersonnel.findMany({
        where: { subcontractorId },
        include: { certifications: true, jobAssignments: true, documents: true }
    });
    return personnel;
}
async function addSubcontractorPersonnel(subcontractorId, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can manage personnel' };
    const sub = await prisma_1.prisma.subcontractor.findUnique({ where: { id: subcontractorId } });
    if (!sub)
        throw { status: 404, message: 'Subcontractor not found' };
    return await prisma_1.prisma.subcontractorPersonnel.create({
        data: {
            subcontractorId,
            name: data.name.trim(),
...
async function updateSubcontractorPersonnel(id, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr' && userRole !== 'supervisor')
        throw { status: 403, message: 'Only Owner, HR, or Supervisor can update personnel' };
    return await prisma_1.prisma.subcontractorPersonnel.update({
        where: { id },
        data: {
            ...(data.name !== undefined && { name: data.name.trim() }),
...
async function removeSubcontractorPersonnel(id, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Only Owner or HR can remove personnel' };
    await prisma_1.prisma.subcontractorPersonnel.delete({ where: { id } });
    return { message: 'Deleted' };
}
async function addPersonnelJobAssignment(subcontractorId, personnelId, jobId, userId, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr' && userRole !== 'supervisor') {
        throw { status: 403, message: 'Only Owner, HR, or assigned Supervisor can assign jobs' };
    }
    const personnel = await prisma_1.prisma.subcontractorPersonnel.findUnique({
        where: { id: personnelId },
        select: { id: true, subcontractorId: true },
    });
    if (!personnel)
        throw { status: 404, message: 'Personnel not found' };
    if (personnel.subcontractorId !== subcontractorId) {
        throw { status: 400, message: 'Personnel does not belong to this subcontractor' };
    }
    const subJob = await prisma_1.prisma.subcontractorJobAssignment.findUnique({
    ...
    if (userRole === 'supervisor') {
        const managed = await prisma_1.prisma.jobSupervisor.findFirst({
            where: { jobId, userId },
            select: { id: true },
        });
        if (!managed)
            throw { status: 403, message: 'Only assigned supervisors can manage this job workforce' };
    }
    const existing = await prisma_1.prisma.subcontractorPersonnelJobAssignment.findFirst({
        where: { personnelId, jobId },
    });
    if (existing)
        return existing;
    return await prisma_1.prisma.subcontractorPersonnelJobAssignment.create({
        data: { personnelId, jobId }
    });
}
async function updatePersonnelJobAssignment(id, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Forbidden' };
    const updated = await prisma_1.prisma.subcontractorPersonnelJobAssignment.update({
        where: { id },
    ...
    if (data.orientationCompletedAt) {
        await prisma_1.prisma.subcontractorPersonnel.update({
...
async function removePersonnelJobAssignment(id, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Forbidden' };
    await prisma_1.prisma.subcontractorPersonnelJobAssignment.delete({ where: { id } });
    return { message: 'Deleted' };
}
async function addPersonnelCertification(personnelId, userRole, data, uploaderId, uploaderName) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Forbidden' };
    const personnel = await prisma_1.prisma.subcontractorPersonnel.findUnique({ where: { id: personnelId } });
    if (!personnel)
        throw { status: 404, message: 'Personnel not found' };
    const status = certStatus(data.expiresAt);
    const cert = await prisma_1.prisma.subcontractorPersonnelCertification.create({
        data: {
            personnelId,
            name: data.name.trim(),
            issuedAt: data.issuedAt.trim(),
...
async function updatePersonnelCertification(id, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Forbidden' };
    const existing = await prisma_1.prisma.subcontractorPersonnelCertification.findUnique({ where: { id } });
    if (!existing)
        throw { status: 404, message: 'Not found' };
    const expiresAt = data.expiresAt ?? existing.expiresAt;
    const status = certStatus(expiresAt);
    const updatedCert = await prisma_1.prisma.subcontractorPersonnelCertification.update({
        where: { id },
        data: {
            ...(data.name !== undefined && { name: data.name.trim() }),
            ...(data.issuedAt !== undefined && { issuedAt: data.issuedAt.trim() }),
...
async function removePersonnelCertification(id, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Forbidden' };
    const existing = await prisma_1.prisma.subcontractorPersonnelCertification.findUnique({ where: { id } });
    if (!existing)
        throw { status: 404, message: 'Not found' };
    await prisma_1.prisma.subcontractorPersonnelCertification.delete({ where: { id } });
...
async function addPersonnelDocument(personnelId, userRole, data) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Forbidden' };
    return await prisma_1.prisma.subcontractorPersonnelDocument.create({
        data: {
            personnelId,
            name: data.name,
...
async function removePersonnelDocument(id, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr')
        throw { status: 403, message: 'Forbidden' };
    const doc = await prisma_1.prisma.subcontractorPersonnelDocument.findUnique({ where: { id } });
    if (!doc)
        throw { status: 404, message: 'Not found' };
    await prisma_1.prisma.subcontractorPersonnelDocument.delete({ where: { id } });
    return doc; // return to controller so it can fs.unlink the file
}
async function checkInSubcontractorPersonnel(personnelId, jobId, date, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr' && userRole !== 'supervisor')
        throw { status: 403, message: 'Forbidden' };
    // Check if assignment exists
    const assignment = await prisma_1.prisma.subcontractorPersonnelJobAssignment.findUnique({
        where: { personnelId_jobId: { personnelId, jobId } }
    });
    if (!assignment)
        throw { status: 400, message: 'Worker is not assigned to this job' };
    return await prisma_1.prisma.subcontractorPersonnelCheckIn.create({
        data: {
            personnelId,
            jobId,
            date,
...
async function checkOutSubcontractorPersonnel(personnelId, jobId, date, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr' && userRole !== 'supervisor')
        throw { status: 403, message: 'Forbidden' };
    const existing = await prisma_1.prisma.subcontractorPersonnelCheckIn.findFirst({
        where: { personnelId, jobId, date, checkedOutAt: null },
        orderBy: { checkedInAt: 'desc' }
    });
    if (!existing)
        throw { status: 404, message: 'Not checked in' };
    return await prisma_1.prisma.subcontractorPersonnelCheckIn.update({
        where: { id: existing.id },
        data: { checkedOutAt: new Date() }
    });
}
async function listSubcontractorPersonnelCheckIns(subcontractorId, userRole) {
    if (userRole !== 'owner' && userRole !== 'hr' && userRole !== 'supervisor')
        throw { status: 403, message: 'Forbidden' };
    const personnel = await prisma_1.prisma.subcontractorPersonnel.findMany({
        where: { subcontractorId },
    ...
    if (!personnel.length)
        return [];
    return await prisma_1.prisma.subcontractorPersonnelCheckIn.findMany({
