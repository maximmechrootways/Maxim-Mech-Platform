...
const outgoingInvoiceService_1 = require("../services/outgoingInvoiceService");
...
router.get('/summary', async (req, res, next) => {
    try {
        ...
        res.json(await (0, outgoingInvoiceService_1.summaryOutgoingInvoices)());
    }
    catch (e) {
        next(e);
    }
...
router.get('/admin/status', async (req, res, next) => {
    try {
        ...
        res.json(await (0, outgoingInvoiceIngestionService_1.getOutgoingInvoicePipelineStatus)());
    }
    catch (e) {
        next(e);
    }
...
router.post('/admin/setup-trigger', async (req, res, next) => {
    try {
        ...
        res.json(await (0, outgoingInvoiceIngestionService_1.bootstrapOutgoingInvoiceIntegration)());
    }
    catch (e) {
        next(e);
    }
...
router.post('/admin/sync', async (req, res, next) => {
    try {
        ...
        const poll = await (0, outgoingInvoiceIngestionService_1.pollSentOutgoingInvoiceEmails)();
        const processed = await (0, outgoingInvoiceIngestionService_1.processOutgoingInvoiceQueue)(10);
        const status = await (0, outgoingInvoiceIngestionService_1.getOutgoingInvoicePipelineStatus)();
...
router.patch('/:id', async (req, res, next) => {
    try {
        ...
        res.json(await (0, outgoingInvoiceService_1.updateOutgoingInvoice)(req.params.id, {
...
router.post('/:id/rescan', async (req, res, next) => {
    try {
        ...
        res.json(await (0, outgoingInvoiceService_1.rescanOutgoingInvoiceFromPdf)(req.params.id));
    }
    catch (e) {
        if (e?.status === 404)
            return res.status(404).json({ error: e.message });
...
router.delete('/:id', async (req, res, next) => {
    try {
        ...
        res.json(await (0, outgoingInvoiceService_1.deleteOutgoingInvoice)(req.params.id));
    }
    catch (e) {
        if (e?.status === 404)
            return res.status(404).json({ error: e.message });
...
router.get('/:id/attachments/:attachmentId/file', async (req, res, next) => {
    try {
        ...
        const { buffer, originalName, mimeType } = await (0, outgoingInvoiceService_1.getOutgoingInvoiceAttachmentBuffer)(req.params.id, req.params.attachmentId);
...
router.get('/:id/attachments/:attachmentId/download', async (req, res, next) => {
    try {
        ...
        const result = await (0, outgoingInvoiceService_1.getOutgoingInvoiceAttachmentDownloadUrl)(req.params.id, req.params.attachmentId);
        res.json(result);
    }
    catch (e) {
        if (e?.status === 404)
...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        res.json(await (0, outgoingInvoiceService_1.getOutgoingInvoiceDetail)(req.params.id));
    }
    catch (e) {
        if (e?.status === 404)
            return res.status(404).json({ error: e.message });
...
router.get('/', async (req, res, next) => {
    try {
        ...
        const maxTotal = req.query.maxTotal != null && req.query.maxTotal !== ''
            ...
            : undefined;
        const result = await (0, outgoingInvoiceService_1.listOutgoingInvoices)({
            q,
            customer,
            dateFrom,
            dateTo,
