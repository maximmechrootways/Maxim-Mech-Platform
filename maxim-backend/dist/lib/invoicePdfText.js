...
exports.extractInvoiceAttachmentText = extractInvoiceAttachmentText;
exports.extractInvoicePdfText = extractInvoicePdfText;
exports.cleanVendorName = cleanVendorName;
exports.guessVendorFromPdfText = guessVendorFromPdfText;
exports.guessVendorFromSender = guessVendorFromSender;
exports.guessAmountsFromPdfText = guessAmountsFromPdfText;
exports.isValidInvoiceNumber = isValidInvoiceNumber;
exports.guessInvoiceNumberFromPdfText = guessInvoiceNumberFromPdfText;
exports.isValidPoNumber = isValidPoNumber;
exports.guessPoNumberFromPdfText = guessPoNumberFromPdfText;
exports.extractBasicInvoiceFieldsFromPdfText = extractBasicInvoiceFieldsFromPdfText;
exports.guessTotalFromPdfText = guessTotalFromPdfText;
const document_intelligence_extractor_1 = require("./document-intelligence-extractor");
// eslint-disable-next-line @typescript-eslint/no-require-imports
const pdfParse = require('pdf-parse');
function azureOcrConfigured() {
 ...
 */
async function extractInvoiceAttachmentText(buffer, options) {
...
async function extractInvoicePdfText(buffer) {
...
const VENDOR_STOP_RE = /\s+(?:\d+\s+(?:[A-Z][A-Za-z]*\s+){0,3}(?:ST|STREET|DR|DRIVE|RD|ROAD|AVE|AVENUE|BLVD|BOULEVARD|LANE|WAY|UNIT|#)|PO\s*BOX|Tel:|Fax:|E-?Mail:|GST\/HST|INVOICE\s+NO|Page:|Statement\s+Date|Customer\s*#|Bill\s+To:|RECEIVED\s+BY)/i;
const NOT_VENDOR_RE = /\b(?:maxim\s+mechanical|nobleton\s+lakes|etobicoke|forwarded\s+message)\b/i;
...
function cleanVendorName(raw) {
    ...
    if (inc?.[1] && inc[1].length <= 80)
        name = inc[1].trim();
    if (name.length > 80)
        name = name.slice(0, 80).trim();
    name = name.replace(/[®,]/g, (m) => (m === '®' ? '' : m)).replace(/\s+/g, ' ').trim();
    if (name.length < 2 || NOT_VENDOR_RE.test(name))
        return null;
    return name;
}
function guessVendorFromSubject(subject) {
    if (!subject)
        return null;
...
function guessVendorFromPdfText(text, context) {
    const compact = text.replace(/\s+/g, ' ').trim();
    const candidates = [];
    const subjectVendor = guessVendorFromSubject(context?.subject);
    if (subjectVendor)
        candidates.push(subjectVendor);
    const bodyVendor = guessVendorFromEmailBody(context?.bodyText);
    if (bodyVendor)
        candidates.push(bodyVendor);
    const remit = compact.match(/(?:REMIT\s+TO|SEND\s+ALL\s+PAYMENTS\s+TO):\s*([A-Z0-9][^.]{3,100}?(?:INC|LTD|CORP|LLC|LP|CO)\.?)/i);
    if (remit?.[1])
        candidates.push(cleanVendorName(remit[1]));
    const brandRentals = compact.match(/\b(SUNBELT®?\s*RENTALS(?:\s+OF\s+CANADA)?(?:\s+INC)?\.?)/i);
    if (brandRentals?.[1])
        candidates.push(cleanVendorName(brandRentals[1].replace(/®/g, '')));
    const bdiCanada = compact.match(/\b(BDI\s+Canada\s+Inc\.?)\b/i);
    if (bdiCanada?.[1])
        candidates.push(cleanVendorName(bdiCanada[1]));
    const liftEquipment = compact.match(/\b([A-Z][A-Z\s-]{2,40}LIFT\s+EQUIPMENT)\b/);
    if (liftEquipment?.[1])
        candidates.push(cleanVendorName(liftEquipment[1]));
    const startCompany = compact.match(/^([A-Z][A-Za-z0-9\s&.'-]{2,70}?\b(?:INC|LTD|CORP|LLC|LP)\.?)\b/);
    if (startCompany?.[1])
        candidates.push(cleanVendorName(startCompany[1]));
    const beforeInvoice = compact.match(/^([A-Z][A-Za-z0-9\s&.'-]{2,70}?)\s+INVOICE\b/);
    if (beforeInvoice?.[1])
        candidates.push(cleanVendorName(beforeInvoice[1]));
    const equipmentService = compact.match(/([A-Z][A-Za-z0-9\s&.'-]{2,50}?\b(?:INC|LTD|CORP)\.?)\s+\d{1,5}\s+[A-Za-z]/);
    if (equipmentService?.[1])
        candidates.push(cleanVendorName(equipmentService[1]));
    const valid = candidates.filter((c) => Boolean(c && c.length >= 3 && !NOT_VENDOR_RE.test(c)));
    if (valid.length === 0)
        return null;
    valid.sort((a, b) => {
        const aFromSubject = a === subjectVendor ? 0 : 1;
        const bFromSubject = b === subjectVendor ? 0 : 1;
        if (aFromSubject !== bFromSubject)
            return aFromSubject - bFromSubject;
...
function vendorFromEmailAddress(email) {
    const normalized = email.trim().toLowerCase();
    const domain = normalized.split('@')[1];
    if (!domain)
        return null;
    if (KNOWN_VENDOR_DOMAINS[domain])
        return KNOWN_VENDOR_DOMAINS[domain];
    const base = domain.split('.')[0];
    if (/^(gmail|googlemail|outlook|hotmail|yahoo|maximmech)/.test(base))
        return null;
    if (base.includes('torcan') && base.includes('lift'))
        return 'Torcan Lift Equipment';
    if (base.includes('sunbelt'))
        return 'Sunbelt Rentals';
    if (base.includes('traditional') && base.includes('air'))
        return 'Traditional Air Systems';
    if (base === 'nextsupply' || base.includes('nextsupply'))
        return 'Next Supply';
    if (base === 'bdi-canada' || base.includes('bdi'))
        return 'BDI Canada';
    return null;
}
...
function vendorNameFromDomain(domain) {
    if (!domain)
        return null;
    const base = domain.split('.')[0];
    if (!base || FREE_EMAIL_PROVIDERS.test(base))
        return null;
 ...
 */
function guessVendorFromSender(from) {
    if (!from)
        return null;
    const raw = from.trim();
    ...
    if (display && !display.includes('@')) {
        ...
        if (cleaned && VENDOR_SUFFIX_RE.test(cleaned))
            return cleaned;
    }
    return vendorNameFromDomain(domain);
}
function guessVendorFromEmailBody(body) {
    if (!body)
        return null;
    ...
    if (namedCompany?.[1])
        return cleanVendorName(namedCompany[1]);
    const torcan = body.match(/\b(TORCAN\s+LIFT\s+EQUIPMENT)\b/i);
    if (torcan?.[1])
        return cleanVendorName(torcan[1]);
    return null;
}
function isStatementDocument(text) {
    return /\bStatement\b/i.test(text) && /Balance\s*Due/i.test(text);
}
function parseSubtotalTaxTotalTriple(text) {
    ...
    if (subtotal == null || taxAmount == null || totalAmount == null)
        return null;
    if (Math.abs(totalAmount - (subtotal + taxAmount)) > 0.05)
        return null;
    return { subtotal, taxAmount, totalAmount };
}
/** Extract subtotal, tax, and final total from labeled PDF text. */
function guessAmountsFromPdfText(text) {
    const compact = text.replace(/\s+/g, ' ').trim();
...
function isValidInvoiceNumber(value) {
    ...
    if (trimmed.length < 3 || trimmed.length > 40)
        return false;
    if (INVALID_INVOICE_TOKENS.has(trimmed.toLowerCase()))
        return false;
    if (!/\d/.test(trimmed))
        return false;
    if (/^charge|^balance|^paid/i.test(trimmed))
        return false;
    return true;
}
function guessInvoiceNumberFromPdfText(text, context) {
    if (context?.subject) {
        const stmt = context.subject.replace(/^fwd:\s*/i, '').match(/statement\s+(\d{4,})/i);
        if (stmt?.[1] && isValidInvoiceNumber(stmt[1]))
            return stmt[1].trim();
    }
    const compact = text.replace(/\s+/g, ' ').trim();
    const candidates = [];
...
function isValidPoNumber(value) {
    if (!value)
        return false;
    const trimmed = value.trim();
    if (trimmed.length < 2 || trimmed.length > 30)
        return false;
    if (INVALID_PO_TOKENS.has(trimmed.toLowerCase()))
        return false;
    if (!/\d/.test(trimmed))
        return false;
    return true;
}
function guessPoNumberFromPdfText(text) {
    const compact = text.replace(/\s+/g, ' ').trim();
    const patterns = [
    ...
    for (const pattern of patterns) {
        ...
        if (value && isValidPoNumber(value))
            return value;
    }
    return null;
}
/** Regex-based extraction when Claude is unavailable or fails. */
function extractBasicInvoiceFieldsFromPdfText(text, context) {
    const compact = text.replace(/\s+/g, ' ').trim();
    ...
    const invoiceDateMatch = compact.match(/(?:INVOICE\s*)?DATE\s*(\d{1,2}\/\d{1,2}\/\d{4}|\d{4}-\d{2}-\d{2})/i);
...
function guessTotalFromPdfText(text) {
