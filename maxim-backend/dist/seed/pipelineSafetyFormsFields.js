...
exports.buildPressureTestingChecklistFields = buildPressureTestingChecklistFields;
exports.buildActivePipelineConnectionsHydrocarbonsFields = buildActivePipelineConnectionsHydrocarbonsFields;
exports.buildDrainAndVentTestFormFields = buildDrainAndVentTestFormFields;
function buildPressureTestingChecklistFields() {
    return [
...
function buildActivePipelineConnectionsHydrocarbonsFields() {
    ...
    fields.push({ type: 'TEXT', label: '[SECTION]Sign-off', required: false }, { type: 'TEXT', label: 'Date left in service', required: false }, { type: 'SIGNATURE', label: 'Signature', required: true }, { type: 'DATE', label: 'Sign-off date', required: true });
    return fields;
}
const DRAIN_VENT_TEST_TYPES = ['Standing water', 'Ball', 'Hydrostatic', 'Pneumatic', 'Smoke', 'Visual'];
const WITNESS_ROLES = [
    'Company name',
...
function buildDrainAndVentTestFormFields() {
    const fields = [
    ...
    for (const t of DRAIN_VENT_TEST_TYPES) {
        fields.push({ type: 'TEXT', label: `${t} — Duration`, required: false }, { type: 'TEXT', label: `[DROPDOWN]${t} — Leaks::Yes|No`, required: false }, { type: 'TEXT', label: `${t} — % of Leaks`, required: false }, { type: 'TEXT', label: `[DROPDOWN]${t} — Acceptable::Yes|No`, required: false }, { type: 'TEXT', label: `${t} — Comments`, required: false });
    }
    fields.push({ type: 'TEXT', label: '[SECTION]Test witnessed by', required: false });
    for (const role of WITNESS_ROLES) {
        fields.push({ type: 'TEXT', label: `${role} — print name`, required: false }, { type: 'SIGNATURE', label: `${role} — signature`, required: false }, { type: 'DATE', label: `${role} — date`, required: false });
    }
    return fields;
}
