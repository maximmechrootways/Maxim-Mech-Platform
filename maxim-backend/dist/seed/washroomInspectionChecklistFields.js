...
exports.buildWashroomInspectionChecklistFields = buildWashroomInspectionChecklistFields;
const WASHROOM_ITEMS = [
...
function buildWashroomInspectionChecklistFields() {
    const fields = [
    ...
    for (const row of WASHROOM_ITEMS) {
        fields.push({ type: 'CHECKBOX', label: `[WASHROOM_ITEM]${row.item}::${row.description}`, required: false }, { type: 'TEXT', label: `[WASHROOM_NOTES]${row.item}`, required: false });
    }
    fields.push({ type: 'TEXT', label: '[SECTION]Contact / Follow-up', required: false }, { type: 'TEXT', label: 'After completing your inspection, are there any concerns/observations that need to be escalated:', required: false }, { type: 'TEXT', label: '[SECTION]Inspection Sign-off', required: false }, { type: 'DATE', label: 'Date of Inspection', required: true }, { type: 'TEXT', label: 'Time', required: true }, { type: 'TEXT', label: 'Facility/Location', required: true }, { type: 'TEXT', label: 'Name of Inspector', required: true }, { type: 'SIGNATURE', label: 'Signature', required: true });
    return fields;
}
