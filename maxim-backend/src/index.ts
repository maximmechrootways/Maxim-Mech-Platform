...
import { createCustomTemplate, updateTemplate } from './services/pdfTemplateService'
import { buildLegislativeComplianceEvaluationFields } from './seed/legislativeComplianceEvaluationFields'
import { buildCriticalTaskInventoryRiskRegisterFields } from './seed/criticalTaskInventoryRiskRegisterFields'
import { buildConfinedSpaceEntryPermitFields } from './seed/confinedSpaceEntryPermitFields'
import { buildFallArrestInspectionChecklistFields } from './seed/fallArrestInspectionChecklistFields'
import { buildWashroomInspectionChecklistFields } from './seed/washroomInspectionChecklistFields'
import { buildEquipmentInspectionChecklistFields } from './seed/equipmentInspectionChecklistFields'
import {
    buildActivePipelineConnectionsHydrocarbonsFields,
    buildDrainAndVentTestFormFields,
    buildPressureTestingChecklistFields,
} from './seed/pipelineSafetyFormsFields'
import { buildInterim2PmChecklistFields, INTERIM2_PM_TEMPLATE_NAME } from './seed/interim2PmChecklistFields'

import authRoutes from './routes/auth'
import pdfTemplatesRouter from './routes/pdfTemplates'
import pdfSubmissionsRouter from './routes/pdfSubmissions'
import toolboxTopicsRouter from './routes/toolboxTopics'
import documentRoutes from './routes/documents'
import jobRoutes from './routes/jobs'
import siteRoutes from './routes/sites'
import subcontractorRoutes from './routes/subcontractors'
import offlineSubcontractorFormsRoutes from './routes/offlineSubcontractorForms'
import equipmentRoutes from './routes/equipment'
import userRoutes from './routes/users'
import templateRoutes from './routes/templates'
import submissionRoutes from './routes/submissions'
import signableSubmissionRoutes from './routes/signableSubmissions'
import dailyFormsRoutes from './routes/dailyForms'
import dailyHazardRoutes from './routes/dailyHazard'
import dhaPresetsRoutes from './routes/dhaPresets'
import libraryDocumentRoutes from './routes/libraryDocuments'
import libraryRouter from './routes/library'
import signingRoutes from './routes/signing'
import injuryReportRoutes from './routes/injuryReports'
import certificateRoutes from './routes/certificates'
import trainingCourseTypesRouter from './routes/trainingCourseTypes'
import { ensureTrainingCourseCatalog } from './services/trainingCourseTypeService'
import incidentRoutes from './routes/incidents'
import nearMissRoutes from './routes/nearMisses'
...
import safetyAlertRoutes from './routes/safetyAlerts'
import inspectionRoutes from './routes/inspections'
import complianceCalendarRoutes from './routes/complianceCalendar'
import auditLogRoutes from './routes/auditLog'
import qualityFindingsRoutes from './routes/qualityFindings'
import hrTodoRoutes from './routes/hrTodo'
import googleCalendarRoutes from './routes/googleCalendar'
import notificationRoutes from './routes/notifications'
import composioRoutes from './routes/composio'
import composioWebhookRoutes from './routes/composioWebhook'
import composioInvoiceWebhookRoutes from './routes/composioInvoiceWebhook'
import incomingInvoicesRoutes from './routes/incomingInvoices'
import outgoingInvoicesRoutes from './routes/outgoingInvoices'
import formQrCodeRoutes from './routes/formQrCodes'
import qrResolveRoutes from './routes/qrResolve'
import permissionsRoutes from './routes/permissions'
import inviteRoutes from './routes/invite'
import employeeDocumentRoutes from './routes/employeeDocuments'
import estimationProjectFilesRoutes from './routes/estimationProjectFiles'
import inspectionAttachmentRoutes from './routes/inspectionAttachments'
import frankRoutes from './routes/frank'
import localDocumentRoutes from './routes/localDocuments'
import cadViewerRoutes from './routes/cadViewer'
import formAssignmentRoutes from './routes/formAssignments'
import hazardReviewRoutes from './routes/hazardReview'
import productFeedbackRoutes from './routes/productFeedback'
import timeOffRoutes from './routes/timeOff'
import employeeTimeTrackingRoutes from './routes/employeeTimeTracking'
...
import { migrateLocalFilesToBlob } from './utils/migrateFilesToBlob'
import { backfillLibraryDocumentText } from './utils/backfillExtractedText'
import { startNotificationEmailWorker } from './services/notificationEmailQueue'
import { startIncomingInvoiceWorker } from './services/incomingInvoiceIngestionService'
import { startOutgoingInvoiceWorker } from './services/outgoingInvoiceIngestionService'
import { startOutgoingInvoiceReminderWorker } from './services/outgoingInvoiceReminderService'
import { startExpiryNotificationWorker } from './services/expiryNotificationService'
import { startFormsApprovalDigestWorker } from './services/formsApprovalDigestService'
import { startNicheBakeryReminderWorker } from './services/nicheBakeryReminderService'
...
app.use('/webhooks/composio', express.text({ type: '*/*' }), composioWebhookRoutes)
app.use('/webhooks/composio-invoice', express.text({ type: '*/*' }), composioInvoiceWebhookRoutes)
...
if (process.env.NODE_ENV === 'production') {
    app.use((req, res, next) => {
        if (req.headers['x-forwarded-proto'] !== 'https') {
            // Never redirect POST/PUT/PATCH — browsers/clients drop the body and uploads break.
            if (req.method === 'GET' || req.method === 'HEAD') {
                return res.redirect(301, `https://${req.headers.host}${req.url}`)
            }
            return res.status(400).json({ error: 'HTTPS is required for this request.' })
        }
        next()
    })
}

// Ensure upload directory exists (temp dir preferred for large SDS PDFs on Azure)
...
app.use('/uploads', authenticate, async (req, res, next) => {
    try {
        ...
        if (!blobName) return res.status(400).json({ error: 'No file specified' })

        // Get a short-lived SAS URL and stream the blob content to the client
        const sasUrl = await getBlobSasUrl(blobName, 5)
...
app.use('/documents/library', libraryDocumentRoutes)
app.use('/documents', documentRoutes)
app.use('/api/library', libraryRouter)
app.use('/jobs', jobRoutes)
app.use('/sites', siteRoutes)
app.use('/subcontractors', subcontractorRoutes)
app.use('/offline-subcontractor-forms', offlineSubcontractorFormsRoutes)
app.use('/equipment', equipmentRoutes)
app.use('/users', userRoutes)
app.use('/templates', templateRoutes)
app.use('/submissions', submissionRoutes)
app.use('/signable-submissions', signableSubmissionRoutes)
app.use('/daily-forms', dailyFormsRoutes)
app.use('/daily-hazard-analysis', dailyHazardRoutes)
app.use('/dha-presets', dhaPresetsRoutes)
app.use('/pdf-templates', pdfTemplatesRouter)
app.use('/pdf-submissions', pdfSubmissionsRouter)
app.use('/toolbox-topics', toolboxTopicsRouter)
app.use('/signing', signingRoutes)
app.use('/injury-reports', injuryReportRoutes)
app.use('/certificates', certificateRoutes)
app.use('/training-course-types', trainingCourseTypesRouter)
app.use('/incidents', incidentRoutes)
app.use('/near-misses', nearMissRoutes)
app.use('/hazards', hazardRoutes)
app.use('/observations', observationRoutes)
app.use('/capa', capaRoutes)
app.use('/safety-alerts', safetyAlertRoutes)
app.use('/inspections', inspectionRoutes)
app.use('/compliance-calendar', complianceCalendarRoutes)
app.use('/audit-log', auditLogRoutes)
app.use('/quality-findings', qualityFindingsRoutes)
app.use('/hr-todo', hrTodoRoutes)
app.use('/google-calendar', googleCalendarRoutes)
app.use('/notifications', notificationRoutes)
app.use('/composio', composioRoutes)
app.use('/incoming-invoices', incomingInvoicesRoutes)
app.use('/outgoing-invoices', outgoingInvoicesRoutes)
app.use('/hazard-review', hazardReviewRoutes)
app.use('/feedback', productFeedbackRoutes)
app.use('/time-off', timeOffRoutes)
app.use('/employee-time-tracking', employeeTimeTrackingRoutes)
app.use('/form-qr-codes', formQrCodeRoutes)
app.use('/qr', qrResolveRoutes)
app.use('/permissions', permissionsRoutes)
app.use('/invite', inviteRoutes)
app.use('/employee-documents', employeeDocumentRoutes)
app.use('/estimation-project-files', estimationProjectFilesRoutes)
app.use('/inspection-attachments', inspectionAttachmentRoutes)
app.use('/frank', frankRoutes)
app.use('/local-documents', localDocumentRoutes)
app.use('/cad-viewer', cadViewerRoutes)
app.use('/form-assignments', formAssignmentRoutes)
app.use('/admin', adminRoutes)

// Temporary migration route — remove after running once
app.post('/admin/migrate-to-blob', authenticate, async (req, res) => {
...
app.post('/admin/backfill-doc-text', authenticate, async (req, res) => {
    if (req.user?.role !== 'owner') {
        return res.status(403).json({ error: 'Owner only' })
    }
    try {
        const result = await backfillLibraryDocumentText()
        res.json(result)
    } catch (err: unknown) {
    ...
    try {
        await Promise.race([
            ...
            new Promise<never>((_, rej) => {
                setTimeout(() => rej(new Error('database health check timed out')), 8_000)
            }),
        ])
    } catch {
        dbStatus = 'error'
...
async function seedToolboxTalkNativeTemplate() {
    ...
    const toolboxPdf = await prisma.pdfTemplate.findFirst({
        where: {
            isActive: true,
            OR: [
    ...
    if (!toolboxPdf?.name) return

    const alreadyExists = await prisma.pdfTemplate.findFirst({
        where: {
            isActive: true,
    ...
    const systemOwner = await prisma.user.findFirst({ where: { role: 'owner' }, select: { id: true, role: true } })
    const systemHr = !systemOwner ? await prisma.user.findFirst({ where: { role: 'hr' }, select: { id: true, role: true } }) : null
    const systemUser = systemOwner ?? systemHr
    if (!systemUser?.id || !systemUser.role) return
    ...
    function normalizeCustomFields(inputFields: typeof fields) {
        if (!Array.isArray(inputFields) || inputFields.length === 0) return []
    ...
    if (!alreadyExists) {
        const created = await createCustomTemplate(systemUser.id, systemUser.role, {
            name: toolboxPdf.name,
            description: toolboxPdf.description ?? undefined,
            assignedRoles: toolboxPdf.assignedRoles as string[],
            assignedUserIds: toolboxPdf.assignedUserIds as string[],
            fields,
        })
    ...
    const signatureFieldCount = await prisma.pdfField.count({
        where: {
    ...
    if (signatureFieldCount === attendeeSlots) return

    await prisma.pdfField.deleteMany({ where: { templateId: alreadyExists.id } })
...
async function seedNearMissNativeTemplate() {
    ...
    const TEMPLATE_DESCRIPTION =
        'Report a near-miss event: site, description, corrective action, and completion details. Submissions appear under Forms & Documents and Health & Safety.'
    ...
    if (alreadyExists?.id) {
        await updateTemplate(alreadyExists.id, systemUser.id, systemUser.role, {
            name: TEMPLATE_NAME,
...
async function seedWeeklyProjectInspectionNativeTemplate() {
    ...
    const weeklyPdf = await prisma.pdfTemplate.findFirst({
        where: {
            isActive: true,
            AND: [
    ...
    if (alreadyExists?.id) {
        const fieldCount = await prisma.pdfField.count({ where: { templateId: alreadyExists.id } })
    ...
    function encodeDropdownLabel(question: string, options: string[]) {
    ...
    for (let i = 1; i <= attendeeSlots; i++) {
        fields.push(
            { type: 'TEXT', label: `Hazard Row ${i}: Item # / Location`, required: false },
            { type: 'TEXT', label: `Hazard Row ${i}: Hazards Observed`, required: false },
            { type: 'TEXT', label: encodeDropdownLabel(`Hazard Row ${i}: Likelihood (A/B/C)`, ['A', 'B', 'C']), required: false },
            { type: 'TEXT', label: `Hazard Row ${i}: Corrective Measures taken/suggested`, required: false },
            { type: 'TEXT', label: encodeDropdownLabel(`Hazard Row ${i}: Repeat (Y/N)`, ['Y', 'N']), required: false },
            { type: 'DATE', label: `Hazard Row ${i}: Date Resolved`, required: false },
            { type: 'TEXT', label: `Hazard Row ${i}: Comments / Follow-Up / Site Incidents or Near Misses Reported`, required: false },
        )
    }
    ...
    if (alreadyExists?.id) {
        ...
        if (fieldCount >= Math.max(10, Math.floor(expectedCount * 0.8))) return
        ...
        function normalizeCustomFields(inputFields: Array<{ type: string; label: string; required: boolean }>) {
            if (!Array.isArray(inputFields) || inputFields.length === 0) return []
            return inputFields.map((f, idx) => {
                const rawType = String(f?.type ?? 'TEXT').toUpperCase()
                const type = (rawType === 'DATE' || rawType === 'SIGNATURE' || rawType === 'CHECKBOX' || rawType === 'NUMBER' ? rawType : 'TEXT') as any
...
async function seedPowerElevatingWorkPlatformsTemplate() {
    ...
    if (alreadyExists?.id) {
        const hasLiftField = await prisma.pdfField.findFirst({
            where: { templateId: alreadyExists.id, label: LIFT_NUMBER_FIELD_LABEL },
            select: { id: true },
        })
        if (hasLiftField) return

        const operatorField = await prisma.pdfField.findFirst({
            where: { templateId: alreadyExists.id, label: 'Operator' },
        })
        const dateField = await prisma.pdfField.findFirst({
            where: { templateId: alreadyExists.id, type: 'DATE', label: 'Date' },
        })
        if (!operatorField || !dateField) {
            console.warn(
                `Power Elevating / Work Platforms template ${alreadyExists.id}: could not add "${LIFT_NUMBER_FIELD_LABEL}" (Operator or Date field missing).`
            )
            return
        }
        ...
        console.log(
            `Power Elevating / Work Platforms template: added missing field "${LIFT_NUMBER_FIELD_LABEL}" (${alreadyExists.id})`
        )
        return
    }
...
async function seedEquipmentInspectionChecklistTemplate() {
    ...
    const TEMPLATE_DESCRIPTION =
        'Pre-operational equipment inspection checklist (daily). Sections: General, Tires, Operating System, Fluids & Belts, Fuel, Steering, Lift System, Brakes, Gauges.'
    ...
    if (existingTemplates.length > 0) {
        for (const tpl of existingTemplates) {
            await updateTemplate(tpl.id, systemUser.id, systemUser.role, {
                name: TEMPLATE_NAME,
                description: TEMPLATE_DESCRIPTION,
                assignedRoles: ['labourer', 'supervisor'],
                fields,
...
async function seedLegislativeComplianceEvaluationTemplate() {
    ...
    const TEMPLATE_DESCRIPTION =
        'Legislative compliance evaluation (Ontario). Use the downloadable checklist for full citation text. Minimum annual completion; review yearly (e.g. COR 2020 Element 13.4). Link submissions to the relevant job in Job Management.'

    const fields = buildLegislativeComplianceEvaluationFields()
...
async function seedCriticalTaskInventoryRiskRegisterTemplate() {
    ...
    const fields = buildCriticalTaskInventoryRiskRegisterFields()

    const created = await createCustomTemplate(systemUser.id, systemUser.role, {
        name: TEMPLATE_NAME,
        description:
            'Critical task inventory and risk register. Use the downloadable PDF (V.2) for full structure and guidance. Link each submission to the relevant job in Job Management.',
        assignedRoles: ['owner', 'hr', 'supervisor', 'labourer'],
        fields,
    })
...
async function seedConfinedSpaceEntryPermitTemplate() {
    ...
    const TEMPLATE_DESCRIPTION =
        'Confined space entry permit (2026 form). Use the downloadable PDF for full layout. Link each submission to the relevant job in Job Management.'
    ...
    const fields = buildConfinedSpaceEntryPermitFields()
...
async function seedFallArrestInspectionChecklistTemplate() {
    ...
    const TEMPLATE_DESCRIPTION =
        'Pre-use daily fall-arrest equipment inspection checklist. Complete one daily checklist section and add comments/notes.'
    ...
    const fields = buildFallArrestInspectionChecklistFields()
...
async function seedWashroomInspectionChecklistTemplate() {
    ...
    const GENERIC_DESCRIPTION =
        'Daily washroom inspection: choose washroom location, then Yes/No/N/A per item with notes. Defaults apply from the location you pick.'
    ...
    const fields = buildWashroomInspectionChecklistFields()
    const roles = ['owner', 'hr', 'supervisor', 'labourer'] as const

    async function upsertWashroomTemplate(name: string, description: string) {
        ...
        if (existing) {
            await updateTemplate(existing.id, actorId, actorRole, {
                name,
                description,
                assignedRoles: [...roles],
                fields,
        ...
        const created = await createCustomTemplate(actorId, actorRole, {
            name,
            description,
            assignedRoles: [...roles],
            fields,
...
const PIPELINE_FORM_ROLES = ['labourer', 'supervisor'] as const

async function seedPressureTestingChecklistTemplate() {
    const CUSTOM_TEMPLATE_PREFIX = 'custom-form://'
    const TEMPLATE_NAME = 'Pressure Testing Checklist'
    const TEMPLATE_DESCRIPTION =
        'Pressure test parameters, equipment gauge, environmental controls, results, and mechanic/inspector sign-off.'
    ...
    const fields = buildPressureTestingChecklistFields()
    if (existingTemplates.length > 0) {
        for (const tpl of existingTemplates) {
            await updateTemplate(tpl.id, systemUser.id, systemUser.role, {
                name: TEMPLATE_NAME,
...
async function seedActivePipelineConnectionsHydrocarbonsTemplate() {
    ...
    const TEMPLATE_DESCRIPTION =
        'Project/system details, hydrocarbon system type, composition, connection procedure (Yes/No), and sign-off.'
    ...
    const fields = buildActivePipelineConnectionsHydrocarbonsFields()
    if (existingTemplates.length > 0) {
        for (const tpl of existingTemplates) {
            await updateTemplate(tpl.id, systemUser.id, systemUser.role, {
                name: TEMPLATE_NAME,
...
async function seedDrainAndVentTestFormTemplate() {
    ...
    const TEMPLATE_DESCRIPTION =
        'System description, test matrix (standing water through visual), and witnessed-by signatures.'
    ...
    const fields = buildDrainAndVentTestFormFields()
    if (existingTemplates.length > 0) {
        for (const tpl of existingTemplates) {
            await updateTemplate(tpl.id, systemUser.id, systemUser.role, {
                name: TEMPLATE_NAME,
...
async function seedNicheBufferTanksTemplate() {
    ...
    const TEMPLATE_DESCRIPTION =
        'Niche Bakery quarterly buffer tanks inspection with Add In, Location, Line, matrix, notes/comments, and sign-off.'
...
async function seedNicheAirSeparatorsTemplate() {
    ...
    const TEMPLATE_DESCRIPTION =
        'Niche Bakery quarterly air separators inspection with Add In, Location, Line, matrix, notes/comments, and sign-off.'
...
async function seedNicheExpansionTanksTemplate() {
    ...
    const TEMPLATE_DESCRIPTION =
        'Niche Bakery quarterly expansion tanks inspection with Add In, Location, Line, matrix, notes/comments, and sign-off.'
...
async function seedNichePumpsTemplate() {
    ...
    const TEMPLATE_DESCRIPTION =
        'Niche Bakery quarterly pumps inspection with Add In, Location, Line, matrix, notes/comments, and sign-off.'
...
async function seedTestingVerificationProcedureDefTemplate() {
    ...
    const TEMPLATE_DESCRIPTION =
        'Large multi-page DEF testing and verification checklist with component matrix, section separators, and sign-off.'
...
async function seedInterim2PmChecklistTemplate() {
    const CUSTOM_TEMPLATE_PREFIX = 'custom-form://'
    const TEMPLATE_NAME = INTERIM2_PM_TEMPLATE_NAME
    const TEMPLATE_DESCRIPTION =
        'VIA RAIL TMC INTERIM 2 preventative maintenance checklist (19 pages: DEF, Sanding, and WWF systems). Site copy V1.1 — reference PDF available for download.'
    ...
    const fields = buildInterim2PmChecklistFields()
...
async function ensureCoreFormRoleAccess() {
    ...
    const targetTemplates = await prisma.pdfTemplate.findMany({
        where: {
            isActive: true,
            OR: [
...
async function startServer() {
    ...
    try {
        ...
        console.log('Training course catalog ready')
    } catch (err) {
        console.error('Training course catalog seed failed:', err)
    }
    ...
    startIncomingInvoiceWorker()
    startOutgoingInvoiceWorker()
    startOutgoingInvoiceReminderWorker()
    startExpiryNotificationWorker()
    startFormsApprovalDigestWorker()
    startNicheBakeryReminderWorker()
}
