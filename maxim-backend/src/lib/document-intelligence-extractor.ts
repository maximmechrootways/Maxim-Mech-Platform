...
function getClient(): DocumentAnalysisClient {
    ...
    if (!endpoint || !key) {
        throw new Error(
            'AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT and AZURE_DOCUMENT_INTELLIGENCE_KEY must be set for OCR extraction'
        )
    }
 ...
 */
export async function extractPagesWithOCR(buffer: Buffer): Promise<PageContent[]> {
    ...
    const pages: PageContent[] = result.pages.map((page) => {
