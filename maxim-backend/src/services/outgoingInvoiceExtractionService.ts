import { parseIsoDate } from './incomingInvoiceAiService'

export type OutgoingInvoiceExtraction = {
    customerName?: string
...
export function isMaximOutgoingInvoiceText(text: string): boolean {
    return MAXIM_ISSUER_RE.test(text)
}

/** Insert line breaks before known labels when OCR collapses the layout onto one line. */
export function normalizeMaximInvoiceText(text: string): string {
...
export function formatOutgoingDateOnly(date: Date): string {
...
export function parseMaximInvoiceDate(raw: string | undefined): Date | null {
    if (!raw) return null
...
export function parseOutgoingDateString(raw: string | undefined): Date | null {
    if (!raw) return null
    return parseMaximInvoiceDate(raw)
}

export function computeDueDateFromPaymentTerms(invoiceDate: Date | null, paymentTerms: string | undefined): Date | null {
...
function sliceUntilNextField(value: string): string {
    const match = value.match(NEXT_FIELD_RE)
    if (!match || match.index == null || match.index === 0) return value.trim()
    return value.slice(0, match.index).trim()
}

function trimCustomerCompanyName(value: string): string {
    ...
    if (companySuffix) return companySuffix[1].trim()
    const beforeAttn = line.split(/\s+ATTN:/i)[0]?.trim()
    if (beforeAttn) return beforeAttn
...
function extractCustomerName(text: string): string | null {
    ...
    const firstLine = trimCustomerCompanyName(match[1])
        ...
        .trim()
    if (!firstLine || /maxim\s+mechanical/i.test(firstLine)) return null
    return firstLine
}
...
function extractPaymentTerms(text: string): string | undefined {
    ...
    if (netDaysMatch) return `Net ${netDaysMatch[1]} Days`
    return undefined
}

export function parseMaximOutgoingInvoiceTemplate(text: string): OutgoingInvoiceExtraction | null {
    if (!isMaximOutgoingInvoiceText(text)) return null
...
export function mergeMaximTemplateExtraction(
    extracted: OutgoingInvoiceExtraction,
    pdfText: string,
): OutgoingInvoiceExtraction {
    const template = parseMaximOutgoingInvoiceTemplate(pdfText)
    if (!template) return extracted
    ...
    if (!dueDate && invoiceDate && paymentTerms) {
        ...
        if (due) dueDate = formatOutgoingDateOnly(due)
    }
...
export function guessCustomerFromEmailTo(emailTo: string | undefined): string | null {
...
export function deriveOutgoingInvoiceStatus(input: {
    paidAt?: Date | null
    ...
    if (input.paidAt) {
        if (total != null && paid != null && paid > 0 && paid < total) return 'PARTIAL'
        return 'PAID'
    }
