...
function isSupervisorOrAbove(role: string) {
...
export async function createAssignments(
    ...
    const template = await prisma.pdfTemplate.findUnique({ where: { id: data.templateId } })
    ...
    for (const assigneeId of data.assignedToUserIds) {
        const assignment = await prisma.pdfFormAssignment.create({
            data: {
 ...
 */
export async function listAssignments(userId: string, userRole: string, query?: { status?: string; templateId?: string; assignedToId?: string }) {
    const where: any = {}
    if (query?.status) where.status = query.status
    if (query?.status === 'completed') where.forwardedToHRAt = null
    if (query?.templateId) where.templateId = query.templateId
    if (isOwnerOrHr(userRole) && query?.assignedToId) where.assignedToId = query.assignedToId
    ...
    const list = await prisma.pdfFormAssignment.findMany({
        where,
        include: {
    ...
    return list.map((a) => ({
        ...
        dueDate: a.dueDate,
        recurrence: (a as any).recurrence ?? undefined,
        note: a.note,
        status: a.status,
 ...
 */
export async function getAssignmentById(id: string, userId: string, userRole: string) {
    const a = await prisma.pdfFormAssignment.findUnique({
        where: { id },
        include: {
            template: { select: { id: true, name: true, pageCount: true, filePath: true } },
            assignedTo: { select: { id: true, firstName: true, lastName: true } },
 ...
 */
export async function linkSubmission(assignmentId: string, submissionId: string, userId: string) {
    const assignment = await prisma.pdfFormAssignment.findUnique({ where: { id: assignmentId } })
    if (!assignment) throw { status: 404, message: 'Assignment not found' }
    if (assignment.assignedToId !== userId) throw { status: 403, message: 'Not your assignment' }

    await prisma.pdfFormAssignment.update({
...
export async function reviewAssignment(
    ...
    userId: string,
    userRole: string,
    data: { action: 'reviewed' | 'resubmission_required'; comment?: string }
) {
    if (!isSupervisorOrAbove(userRole)) {
 ...
 */
export async function forwardAssignmentToHR(assignmentId: string, userId: string, userRole: string) {
    if (!isSupervisorOrAbove(userRole)) {
        throw { status: 403, message: 'Only supervisors and owners can forward to HR' }
    }

    const assignment = await prisma.pdfFormAssignment.findUnique({
        where: { id: assignmentId },
 ...
 */
export async function getAssignmentCounts(userId: string, userRole: string) {
    if (userRole === 'supervisor') {
        const pendingReview = await prisma.pdfFormAssignment.count({
            where: { assignedById: userId, status: 'completed', forwardedToHRAt: null },
        })
        const total = await prisma.pdfFormAssignment.count({
            where: { assignedById: userId },
        })
    ...
    if (userRole === 'labourer') {
        const pending = await prisma.pdfFormAssignment.count({
