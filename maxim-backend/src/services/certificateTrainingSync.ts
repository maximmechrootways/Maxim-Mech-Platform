...
export function resolveTrainingExpirationDate(expiresAt: string | null | undefined): string | null {
    const trimmed = expiresAt?.trim()
    return trimmed || null
}

function encodeTrainingDisplayName(courseName: string, hoursCompleted?: number, trainingFacility?: string) {
    const payload = {
...
export async function mirrorTrainingDocumentFromCertificate(
    cert: {
        id: string
        name: string
    ...
    if (!cert.holderUserId) return null
...
export async function mirrorCertificateFromTrainingDocument(
    doc: {
        id: string
        employeeId: string
    ...
    const courseName = doc.displayName?.startsWith(TRAINING_META_PREFIX)
        ? (() => {
              try {
                  const parsed = JSON.parse(doc.displayName.slice(TRAINING_META_PREFIX.length)) as { courseName?: string }
    ...
    if (doc.certificateId) {
        const existing = await prisma.certificate.findUnique({ where: { id: doc.certificateId } })
        if (existing) {
...
export async function deleteLinkedTrainingDocument(certificateId: string) {
    const docs = await prisma.employeeDocument.findMany({ where: { certificateId } })
...
function decodeCourseName(displayName: string | null, fallback: string): string {
    const raw = displayName?.trim() || ''
    if (!raw.startsWith(TRAINING_META_PREFIX)) return raw || fallback
    try {
 ...
 */
export async function reconcileAllCertificateTrainingLinks(): Promise<{
    certificatesLinked: number
    trainingDocsLinked: number
    holderIdsResolved: number
    certificatesCreated: number
    trainingDocsMirrored: number
}> {
    const stats = {
    ...
    const trainingDocs = await prisma.employeeDocument.findMany({
    ...
    for (const doc of trainingDocs) {
        ...
        if (doc.certificateId) {
            const linked = await prisma.certificate.findUnique({ where: { id: doc.certificateId } })
            if (linked) {
        ...
        if (blobKey) {
            const byFile = await prisma.certificate.findFirst({ where: { filePath: blobKey } })
            if (byFile) {
                await prisma.employeeDocument.update({
                    where: { id: doc.id },
        ...
        if (created) stats.certificatesCreated += 1
    }

    const certsAfter = await prisma.certificate.findMany()
    for (const cert of certsAfter) {
        if (!cert.holderUserId) continue
        const linkedDoc = await prisma.employeeDocument.findFirst({
...
export async function deleteLinkedCertificate(employeeDocumentId: string) {
    const cert = await prisma.certificate.findFirst({ where: { employeeDocumentId } })
    if (cert) {
    ...
    if (doc?.certificateId) {
        await prisma.certificate.delete({ where: { id: doc.certificateId } }).catch(() => {})
    }
}
