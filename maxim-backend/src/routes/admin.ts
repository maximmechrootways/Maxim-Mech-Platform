...
import { blobExists, listBlobsByPrefix } from '../services/blobStorageService'
...
router.get('/documents/env-check', async (req, res) => {
    if (req.query.key !== DEBUG_SECRET) return res.status(403).json({ error: 'Bad key' })
    res.json({
...
router.get('/documents/debug-reindex', async (req, res) => {
    ...
    try {
        ...
        for (const doc of docs) {
            try {
                const exists = await blobExists(doc.filePath)
                if (!exists) {
...
router.get('/documents/diagnostics', async (req, res, next) => {
    try {
        ...
        const dbDocs = await prisma.libraryDocument.findMany({
            select: {
                id: true,
                name: true,
                filePath: true,
        ...
        const blobNames = await listBlobsByPrefix('documents/')
