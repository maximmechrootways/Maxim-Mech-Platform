...
export const HAZARD_RISK_TEMPLATE_KEYS = [
  'office_engineer',
  'general_labourer',
  'office_management',
  'plumber',
...
export type HazardRiskTemplateKey = (typeof HAZARD_RISK_TEMPLATE_KEYS)[number]

export const HAZARD_RISK_TEMPLATE_META: Record<
  HazardRiskTemplateKey,
...
const ROLE_LINES: Record<HazardRiskTemplateKey, string> = {
...
function sharedFieldsBeforeRoleExtras(roleLine: string): HraFieldDef[] {
  return [
    ...
    },
    {
      ...
      type: 'TEXT',
      label:
        '[DROPDOWN][RISK]Likelihood of occurrence::1 — Remote — very unlikely to happen|2 — Possible — a chance it could happen|3 — Probable — could happen|4 — Expected — very likely to happen',
      required: true,
    },
    {
      ...
      type: 'TEXT',
      label:
        '[DROPDOWN][RISK]Severity::1 — Minimal — no injury, minor damage|2 — Moderate — first aid, minor illness, property damage up to $1K|3 — Serious — medical aid, lost time, property damage over $5K|4 — Catastrophic — death, critical injury, property damage over $10K',
      required: true,
    },
    {
      ...
      type: 'TEXT',
      label:
        '[DROPDOWN][RISK]Initial risk (inherent, before additional controls)::Low (Green)|Medium (Amber)|High (Red)|Severe — stop work (Black)',
      required: true,
    },
    {
    ...
    },
    {
      ...
      type: 'TEXT',
      label:
        '[DROPDOWN][RISK]Residual risk (after controls)::Low (Green)|Medium (Amber)|High (Red)|Severe — do not proceed (Black)',
      required: true,
    },
    {
      stableId: 'ppe',
...
function signoffFields(): HraFieldDef[] {
  return [
...
const ROLE_SPECIFIC_FIELDS: Record<HazardRiskTemplateKey, HraFieldDef[]> = {
  office_engineer: [
    {
...
export function getHazardRiskTemplateFields(key: string): HraFieldDef[] | null {
  if (!HAZARD_RISK_TEMPLATE_KEYS.includes(key as HazardRiskTemplateKey)) return null
  const k = key as HazardRiskTemplateKey
  const core = sharedFieldsBeforeRoleExtras(ROLE_LINES[k])
  const extra = ROLE_SPECIFIC_FIELDS[k] ?? []
  return [...core, ...extra, ...signoffFields()]
}

export type HraFieldWithId = HraFieldDef & { id: string }

export function getHazardRiskTemplateFieldsWithIds(key: string): HraFieldWithId[] | null {
...
export function listHazardRiskTemplates() {
  return HAZARD_RISK_TEMPLATE_KEYS.map((key) => ({
    key,
    ...HAZARD_RISK_TEMPLATE_META[key],
  }))
}
