...
export function buildCriticalTaskInventoryRiskRegisterFields(): Array<{ type: string; label: string; required: boolean }> {
  const fields: Array<{ type: string; label: string; required: boolean }> = [
    ...
    { type: 'TEXT', label: 'Department / area', required: false },
    {
      type: 'TEXT',
      label:
        'Instructions: Identify critical tasks, hazards, and controls. Align entries with the reference PDF. Link this submission to a job using the selector above.',
      required: false,
    },
  ]
  ...
  fields.push(
    { type: 'TEXT', label: '[SECTION]Summary & register maintenance', required: false },
    { type: 'TEXT', label: 'Overall register notes (change history, communication)', required: false },
    { type: 'DATE', label: 'Next review date (register)', required: false },
    {
      type: 'CHECKBOX',
      label: 'Changes communicated to affected workers (where required)',
      required: false,
    },
    { type: 'TEXT', label: '[SECTION]Approval (optional)', required: false },
    { type: 'TEXT', label: 'Approved by (name)', required: false },
    { type: 'SIGNATURE', label: 'Approved by — Signature', required: false },
    { type: 'DATE', label: 'Approval date', required: false }
  )
