...
import { fetchSites, fetchJobs, type JobListItem } from '@/api/jobs'
...
function resetFileInput() {
  const input = document.getElementById('ppd-upload-file') as HTMLInputElement | null
  if (input) input.value = ''
}

export function PastProjectDirectory() {
  const [sites, setSites] = useState<{ id: string; name: string }[]>([])
  const [archivedJobs, setArchivedJobs] = useState<JobListItem[]>([])
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([fetchSites(false), fetchJobs()])
      .then(([siteRows, jobs]) => {
        if (!cancelled) setSites(siteRows.map((s) => ({ id: s.id, name: s.name })))
        if (!cancelled) setArchivedJobs(jobs.filter((j) => j.status !== 'active'))
      })
      .catch(() => {
        if (!cancelled) {
          setSites([])
  ...
  const topRow = PAST_PROJECT_FOLDER_SLUGS.slice(0, 3)
  const bottomRow = PAST_PROJECT_FOLDER_SLUGS.slice(3, 6)
    ...
    <div className="space-y-8 max-w-5xl animate-fade-in">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
            Organize completed project documents into folders. Open a folder to review files, or add a new file here.
          </p>
          <div className="mt-4">
            <Link to="/estimating/current-projects">
      ...
      {showAddForm && (
        <Card padding="lg">
          ...
          <form onSubmit={submit} className="mt-6 space-y-4 max-w-xl">
            <Input label="Name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Contract — Phase 2" required />
            <div>
              <label htmlFor="ppd-folder" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">
                Folder
              </label>
              <select
                id="ppd-folder"
            ...
            <div>
              <label htmlFor="ppd-site" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">
                Job site (optional)
              </label>
              <select
                id="ppd-site"
            ...
            <Textarea
              ...
              placeholder="e.g. Final as-built set; includes RFI responses through closeout."
              rows={4}
              className="min-h-[88px]"
            />
            <div>
              <label htmlFor="ppd-upload-file" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1.5">
                File
              </label>
              <input
                id="ppd-upload-file"
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {topRow.map((f) => (
            <Link
              key={f.slug}
              to={`/estimating/past-project-directory/${f.slug}`}
      ...
      <div>
        ...
        {archivedJobs.length === 0 ? (
          <p className="text-sm text-neutral-500">No inactive jobs yet.</p>
        ) : (
          <ul className="space-y-2">
            {archivedJobs.map((job) => (
              <li key={job.id}>
                <Link
                  to={`/estimating/past-project-directory/job/${job.id}`}
                  className="flex items-center justify-between rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900/60 px-4 py-3 text-sm hover:border-brand-400 hover:bg-brand-50/60 dark:hover:bg-brand-950/30 transition-colors"
                >
                  <span className="text-neutral-800 dark:text-neutral-100">
                    {job.siteName} · {job.title}
                  </span>
                  <span className="text-neutral-500 dark:text-neutral-400">{job.status}</span>
                </Link>
              </li>
            ))}
          </ul>
