...
export function PastProjectFolderPage() {
  ...
  useEffect(() => {
    const def = pastProjectFolderFromSlug(folderSlug)
    if (!def) {
      setRows([])
      setLoading(false)
      return
  ...
  if (!folderDef) {
    return (
      <div className="max-w-lg">
        ...
        <Link to="/estimating/past-project-directory" className="text-sm text-brand-600 dark:text-brand-400 hover:underline mt-2 inline-block">
          ← Back
        </Link>
      </div>
    )
  ...
  return (
    <div className="space-y-6 max-w-4xl animate-fade-in">
      <div className="flex flex-wrap items-center gap-3">
        <Link to="/estimating/past-project-directory" className="text-sm text-brand-600 dark:text-brand-400 hover:underline">
      ...
      {loading ? (
        ...
        <p className="text-neutral-500 text-sm border border-dashed border-neutral-300 dark:border-neutral-600 rounded-xl p-8 text-center">
          No files yet. Add one from the main Past Project Directory page.
        </p>
      ) : (
