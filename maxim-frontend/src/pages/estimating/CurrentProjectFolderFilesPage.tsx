...
import { pastProjectFolderFromSlug } from '@/estimating/pastProjectFolders'
...
export function CurrentProjectFolderFilesPage() {
  const { jobId, folderSlug } = useParams<{ jobId: string; folderSlug: string }>()
  const location = useLocation()
  const folderDef = pastProjectFolderFromSlug(folderSlug)
  const { user } = useUser()
  ...
  useEffect(() => {
    if (!jobId || !folderDef) return
    let cancelled = false
    setLoading(true)
    setError(null)
    fetchJobDetail(jobId)
      .then(async (job) => {
        const files = await fetchEstimationProjectFiles({ folder: folderDef.api, siteId: job.siteId })
        return { job, files }
      })
      .then(({ job, files }) => {
        if (cancelled) return
  ...
  const backPath = isPastContext ? `/estimating/past-project-directory/job/${jobId}` : `/estimating/current-projects/${jobId}`
  ...
  return (
    <div className="space-y-6 max-w-4xl animate-fade-in">
      ...
      <div>
        ...
        <p className="text-sm text-neutral-600 dark:text-neutral-400 mt-1">{siteName || 'Current project site'} files in this folder.</p>
      </div>
      ...
      {loading ? (
        ...
        <p className="text-neutral-500 text-sm border border-dashed border-neutral-300 dark:border-neutral-600 rounded-xl p-8 text-center">
          No files yet for this job site and folder.
        </p>
      ) : (
